...
- **§2.5** Commit 节律
- **§3** Phase / Task 工作流
- **§3.5** 主 agent 自驱（`/goal`）— 长任务跨多轮自治
- **§4** PR 合入流程（PR-only + phase smoke gate） ／ **§4.1** Rebase 同步协议
- **§5** 异常处理
- **§7** 拓扑健康检查
- **§8** 卡住与豁免协议
- **降级到 solo** ／ **参考（项目内自包含）**
...
主 agent / 内部 subagent / CI 脚本跑 baseline / verification 前，**先 source 一次本块**。
...
| # | 目录 | 分支 | 角色 | Owner | 启动门槛 |
|---|---|---|---|---|---|
| 0 | `ContextForge/` (主 repo) | `master` | 协调中心 / merge PR / phase smoke gate | **主 agent** | — |
| 1 | `ContextForge-wt-foundation/` | `feat/foundation` | Phase 1 foundation 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ✅ 立即（无前置）|
| 2 | `ContextForge-wt-index-core/` | `feat/index-core` | Phase 2 index-core 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 1 merge |
| 3 | `ContextForge-wt-agent-importers/` | `feat/agent-importers` | Phase 3 agent-importers 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 1 merge（可与 Phase 2 并行）|
| 4 | `ContextForge-wt-retrieval-explain/` | `feat/retrieval-explain` | Phase 4 retrieval-explain 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 2 merge |
| 5 | `ContextForge-wt-memoryops/` | `feat/memoryops` | Phase 5 memoryops 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 2 + Phase 3 merge（可与 Phase 4 并行）|
| 6 | `ContextForge-wt-cli-api-export/` | `feat/cli-api-export` | Phase 6 cli-api-export 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 4 + Phase 5 merge |
| 7 | `ContextForge-wt-mcp-adapter/` | `feat/mcp-adapter` | Phase 7 mcp-adapter 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 6 merge |
| 8 | `ContextForge-wt-eval-and-reliability/` | `feat/eval-and-reliability` | Phase 8 eval-and-reliability 工作区 | 主 agent（Agent tool, `isolation: "worktree"`）| ⏳ 等 Phase 6 + Phase 7 merge |

> ⚠️ **本仓库主干分支 = `master`**（stock `git init` 默认）。本文件下文所有协议命令里出现的 `main` / `origin/main` 均指本仓库主干 `master` —— 按 §2 开头「分支命名约定」note，执行时整体替换为 `master`。

**并发拓扑**（主 agent 决定 spawn 顺序；并行峰值受 Agent tool 单 message 多 spawn + 主 agent token 预算约束）：
...
新增 worktree 命名规则：phase 级 `ContextForge-wt-<phase-name>/` + 分支 `feat/<phase-name>`；task 级 `ContextForge-wt-task-<X.Y>/` + 分支 `feat/task-<X.Y>-<name>`（worktree 目录用 X.Y、分支含 name — Agent tool `isolation: "worktree"` spawn subagent 时自动落盘）。
...
### R4 · 同一 worktree 同时只能有 1 个 writer

不同 worktree 可并发（主 agent 可并行 spawn 多个 Agent tool subagent 跨 worktree），同一 worktree 不可。
若需在同 phase 内开多个 task，每个 task 用独立 worktree（不同 task 分支）。
...
- Agent tool subagent 在主 repo 做写操作（subagent 应通过 `isolation: "worktree"` 在隔离 worktree 工作）

✅ **允许操作**（明示放行清单）：

| 类别 | 命令 | 执行者 |
|---|---|---|
| 合 PR | `git merge --no-ff <feat-branch>` | 主 agent 在主 repo |
| Tag / Release | `git tag` / `git push origin <tag>` | 主 agent |
| 创建 branch | `git branch <feat-X>` / `git checkout -b <feat-X>` | 主 agent / subagent 在自己 worktree |
| Worktree 管理 | `git worktree add/remove/list` | 主 agent |
| 只读 | `git status` / `git log` / `git diff` / `git show` / `git fetch` | 主 agent / subagent |
...
|---|---|---|
| subagent 同步 main | `git fetch origin && git rebase origin/main` | `git rebase main`（worktree 共享 .git）|
| subagent 完成产出 | `git push -u origin <branch>` + return ready 对象给主 agent | return ready 对象给主 agent（含 branch + commits 信息）|
...
| 跨 worktree subagent rebase 协调 | comment + GitHub UI | §4.1 协议：主 agent merge 后 kill 旧 subagent + 在新 HEAD 上重 spawn |
...
2. 主 agent 走完整 R6 PR 流程加依赖：
   ...
   ```
3. dep merge 后主 agent 杀掉原 task subagent，在新 master HEAD 上重 spawn（不在 live subagent worktree 上 in-place rebase）

> **可 override**：完全互信、无供应链审计需求 → adapter §Workflow Overrides 设 `lockfile-protect: false`（不推荐，治理审计链断）。

---

## 2.5 Commit 节律

主 agent / subagent 实施 task 时在 feature branch 上至少产出：
...
if git remote -v | grep -q "^origin"; then
  ...
  git rebase origin/main || { echo "rebase 冲突 → subagent return blocked-rebase 对象 / 主 agent 自身则转 §5 场景 B"; exit 1; }
else
  git rebase main || { echo "rebase 冲突 → subagent return blocked-rebase 对象 / 主 agent 自身则转 §5 场景 B"; exit 1; }
fi
...
#      不依赖 /s2v-implement skill — subagent / 主 agent 手动实施都必须执行此步。
...
# 7. 进入 §4 PR 合入流程
#    主 agent 自身实施：直接走 §4 Gate 0-5
#    subagent 实施：return ready 对象（含 branch / commits / verification 结果）给主 agent，
#                   主 agent 接收后走 §4 Gate 0-5
...
#    无 remote：主 agent 直接 git checkout + merge --no-ff（无 push 步骤）
```
...
Claude Code v2.1.139+ 的 `/goal` 命令让主 agent 设完成条件后跨多轮自治工作，独立 evaluator 模型（默认 Haiku）每轮判定条件是否满足。完整决策与边界见 [ADR-011](docs/decisions/adr-011-single-driver-with-subagents.md)，本节为实操摘要。

**用法**：
```text
/goal <可机器观测的完成条件 + 怎么证明>     # 设目标，立即开跑
/goal                                       # 看状态（已跑轮数 / token / evaluator 上轮 reason）
/goal clear                                 # 提前停（别名 stop/off/reset/none/cancel）
```

**condition 写法红线**（违反任一会让 /goal 不可用或破坏 R6）：

1. **禁含「merged」/「PR 合并」字面** — merge 是主 agent 显式动作（§4 Gate 0-5），不能交 evaluator 判完成。condition 终点应停在 "ready to merge"（如 §6 AC 全 [x] + §9 全绿 + §10 6 项齐），主 agent 看完成态再手动合
2. **用 §0 helper 表达验证** — condition 必须能被 Claude 自身输出"证明"（evaluator 不跑工具、不读文件，只看 conversation surface）。优先用 `s2v_verify_full "$VERIFY_KEYS"` / `s2v_run unit-test required` 等 helper 跑出可见结果；避免 inline bash 把验证藏到 evaluator 看不到的子进程里
3. **包含 turn 兜底** — 加 "or stop after N turns" 防 condition 无限不可满足时空转 token

**condition 模板**（task 实施场景）：

```text
/goal task-<X.Y> §6 AC1-N 全 [x] 且 §9 verification 全绿（s2v_verify_full $VERIFY_KEYS 0 失败）
     且 §10 Completion Notes 按 standard.md §8.3 6 项 schema 齐 且 §7 追踪表所有行 Done/Waived
     或 25 轮后 stop（届时 return 当前状态摘要给我）
```
...
# Gate 0：先回收该 task 的 worktree —— subagent 已 return ready 对象 / 其 commit
#         在 feature branch 上（worktree 与主 repo 共享 .git），移除 worktree **不丢任何提交**；
#         不先移除则下面 Gate 1 的 `git checkout feat/...` 会撞
#         `fatal: '...' is already checked out at '../ContextForge-wt-task-<X.Y>'`
#         （Agent tool isolation: "worktree" 建的 worktree 仍占用该分支）。
...
case "$SPEC_STATUS" in
  Done|Waived)
    ...
    ;;
  Blocked)
    echo "🛑 STOP: Spec Status=Blocked — 不允许合 PR，先解决 §8 卡住协议（subagent return 的 blocked 对象 / 主 agent 自身 blocked）"
    exit 1
    ;;
  *)
...
## 4.1 Rebase 同步协议（单驱动变体精简）

主 agent 决定 spawn 顺序时遵循以下原则（替代旧的 worktree 终端通知机制）：

1. **串行优先**：依赖前序 merge 的 task → 等前序 merge 完再 spawn 后续 subagent。
2. **并行时单 PR 周期内不跨 merge**：同时活跃的 subagent 应在同一 master HEAD baseline 上工作；若其中一个 PR merge 进 master → **杀掉**其余 subagent，在新 HEAD 上**重 spawn**（不在 live subagent worktree 上 in-place rebase — Agent tool subagent context 在 spawn 时固化）。
3. **长 subagent (>1 PR cycle) 禁止跨 merge**：发现 subagent 工作跨越多个 merge 时，主 agent 必须 kill + respawn，永不 in-place rebase live worktree。
4. **STATUS-MAIN.md 机制已退役**（ADR-011） — 单驱动下主 agent 在自己 context 内可见所有状态，无需文件中转。
...
按 §4.1 单 PR 周期内不跨 merge 原则，rebase 冲突通常不发生。若 subagent 仍遇到冲突：

1. subagent return blocked-rebase 对象给主 agent（含冲突文件路径 + 当前分支 HEAD + 建议解决方向）
2. 主 agent 在 worktree 内手动解冲突 → continue subagent 或 kill + respawn
...
# task 级 worktree 由 Agent tool spawn subagent 时 isolation: "worktree" 参数自动建
# （../ContextForge-wt-task-<X.Y> + feat/task-<X.Y>-<name>），主 agent 通常无需手动建
```
...
主 agent 自身实施或 subagent 实施满足以下**全部**条件即视为"卡住"：
...
2. **subagent 卡住** → return blocked 对象给主 agent（结构化字段如下）；**主 agent 自身卡住** → 在 context 内整理同款字段，转 §主 agent 决策路径：
   ...
   hypothesis: "<我认为根因可能是 X，证据是 Y>"
   options:
     - A: "修改 spec（具体改哪行）"
     - B: "给一个提示让我继续"
     - C: "Waive 该 AC（按 s2v §12.3 五项填写）"
   state:
     red_test: "<test-file>:<line>"
     impl: "<src-file>:<line>"
   ```
...
| 决策 | 主 agent 动作 | 后续 |
|---|---|---|
| 选项 A 改 spec | 主 agent 新开 `chore/spec-fix-task-X.Y` branch → 改 task spec → 自 PR 自 merge → 在新 HEAD 重 spawn subagent | subagent 接力 |
| 选项 B 给提示 | 主 agent 在新 prompt 中带上提示 + 上次 attempts 摘要重 spawn subagent | subagent 接力 |
| 选项 C Waive | 主 agent 改 task spec Status → Waived + 在 §10 Completion Notes 追加 **Waiver 登记**（§12.3 五项展开，模板见 standard.md §10）+ **规范化 §10 其余 6 项**（见下方留痕要求）→ 自 PR 自 merge | 跳过该 AC |
...
> 这些文件是 `/s2v-init` 时从全局 skill（步 0 `_s2v_skill_dir` resolver 解析路径；Claude Code 默认 `~/.claude/skills/s2v/`，其他工具见 `docs/s2v/standard.md` §22）复制的快照，让未来贡献者 / CI / subagent 即使没安装全局 skill 也能读到完整规范。
