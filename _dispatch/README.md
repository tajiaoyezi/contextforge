...
[ADR-011](../docs/decisions/adr-011-single-driver-with-subagents.md) 决策后，项目治理转入"主 agent + 内部 subagent"单驱动模式，外部 worker 终端整套退役。本目录原"worker 派工 prompt 留痕"作用消失，仅保留两类内容：
...
3. review 过 → 主 agent 跑 AGENTS.md §4 Gate 0-5 merge PR；review 未过 → 主 agent 决定修复路径继续

## 边界

- 治理骨架不变：R6 PR-only / R7 lockfile-protect / worktree 拓扑 / ADR 制度 / AGENTS §4 Gate 0-5 全保留
...
- 详细决策与历史背景：[ADR-011](../docs/decisions/adr-011-single-driver-with-subagents.md)
