#!/usr/bin/env bash
# scripts/release_smoke.sh — v0.1 (task-8.3) + v0.2 (task-9.5) + v0.3 (task-10.6) + v0.4 (task-11.4) + v0.9 (task-16.4) release smoke gate.
#
# Sections:
...
echo "release_smoke[5/6]: phase 11 Console Real Data Plane smoke (REAL mode default; LOCAL_ONLY=1 for v0.3 inmem fallback; v7 includes Phase 16 steps 25-27)"
...
echo "release_smoke: tarball_contract=ok smoke_evidence=ok benchmark_gate=ok grpc_search_smoke=ok phase9_cli_e2e=ok phase11_console_real=${RELEASE_SMOKE_CONSOLE:+ok} phase15_console_functional_gap_closure=ok phase16_backlog_completion=${RELEASE_SMOKE_CONSOLE:+ok} phase16_ghcr_verify=${RELEASE_SMOKE_GHCR:+ok}"
