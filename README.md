...
🎯 **v0.38.0 v1.0-api-cli-freeze** — the first step of the v1.0 收口冲刺 (endorsement sprint). **The project never had a v1.0 anchor** (PRD P0 is v0.1's, PRD's "v1.0" is only in the distribution dimension, roadmap has zero v1.0 mentions, README has no maturity label, ADR-017's "v1.0 release gate" was dangling). **ADR-050 formally defines v1.0.0** = 功能成熟度收口 (D1, already met — recall@5/@10=1.0 exceeds PRD north-star 75%/85%) + API/CLI 冻结 (D2, this phase) + 文档对齐 (D3, Phase 46) + GitHub Release 流程 (D4, Phase 46-47). **Not in v1.0** (推 v2.0): multi-user/auth/auto-update/arm64-native. This phase delivers D2: task-45.2 daemon REST **removes 2 v0.1 §2A 501 未实装 endpoints** (`POST /v1/import` + `POST /v1/eval/run`, [BREAKING] — console-api covers both; freezing permanent-501 is unacceptable) + chunk_count honest-defer (Go daemon has no SQLite lib; real counts via console-api `/v1/stats/chunks`); task-45.3 CLI `version` subcommand + top-level `--help`/`-h` (fixes the task-1.4 `-h` → exit 2 hostility) + `contextforge.example.toml` 补全 `[embedding]`/`[vector]`/`[reranker]`/`[retrieval]` 4 retrieval sections. **0 new dependency** / **0 network** / **0 schema migration** / daemon REST removal is a v1.0-pre major-boundary breaking change. **ADR-050 → partial ratify (D1/D2)**; ADR-007 add-only Amendment (v1.0 distribution definition narrowed); ADR-014 — 36th activation.

详 `RELEASE_NOTES.md` v0.38.0 段 + [Phase 45 spec](docs/specs/phases/phase-45-v1.0-api-cli-freeze.md) + [ADR-050](docs/decisions/adr-050-v1.0-definition.md)。

## What's new in v0.37.0

🔒 **v0.37.0 memory-unpin-actor-propagation** — closes the pin/unpin actor-propagation asymmetry Phase 40 task-40.1 left open: `pin` got actor propagation (X-Actor → `PinMemoryRequest.actor` → store `pinned_by`), but `unpin` was missed (`memory.rs:298` hardcoded `"console-api"`). **Grounding found the real value is in audit/event, not the store** (ADR-013): `set_pinned_with_actor(pinned=false)` discards the actor (clears `pinned_by`), so propagating to the store alone is an "empty pass-through"; the real landing point is `emit_audit_and_event` (which didn't accept an actor + hardcoded source). task-44.1 closes the loop: `emit_audit_and_event` gains an `actor` param (audit source AND event source = actor when non-empty; empty → each falls back to its legacy value — `"console-api"` for audit, `"contextforge-core"` for event — byte-equivalent). `unpin` propagates + `pin` 顺带 propagates (消除 pin audit/event 不归因残余不对称). Go side: `handleMemoryUnpin` reads `X-Actor` (mirrors pin), `Unpin(id, actor)` interface, grpcclient `pb.UnpinMemoryRequest.Actor`, memstore signature aligned. proto `UnpinMemoryRequest` add-only `actor=2` (field 1 frozen, ADR-015). **0 new dependency** / **0 network** / **0 schema migration** / **proto add-only** / **default byte-equiv**. **Honest-defer (ADR-013)**: authenticated identity (X-Actor → verified auth subject) needs the console-api auth layer → 🔴 `[SPEC-DEFER:phase-future.memory-actor-authenticated-identity]`; deprecate/softdelete/harddelete actor propagation (7-layer + new migration / audit redesign) → 🔴 `[SPEC-DEFER:phase-future.memory-actor-all-rpc]` (this phase only delivers the shared `emit_audit_and_event` actor-param foundation; those 3 RPCs pass `""` byte-equiv). **ADR-049 → Accepted**; ADR-032/045 add-only Phase-44 Amendment; ADR-014 — 35th activation.

详 `RELEASE_NOTES.md` v0.37.0 段 + [Phase 44 spec](docs/specs/phases/phase-44-memory-unpin-actor-propagation.md) + [ADR-049](docs/decisions/adr-049-memory-unpin-actor-propagation.md)。

## What's new in v0.36.0

🔧 **v0.36.0 governance-debt-cleanup-4 / indexing-replay-splice** — fourth governance-debt sweep, single-focus on the `indexing-replay-e2e` "last mile": Phase 33 task-33.3 (ADR-038 D3) shipped the indexing-event **persistence** (migration 0019 + `SqliteIndexingEventStore` + 4 emit points) **and** the replay **mapper** (`indexing_rows_to_pb_events`, `events.rs:438`, `test_33_3_2`-guarded), but the mapper was **never called on the live subscribe path** (4 splice gaps, grounding verified first-hand): `list(limit)` lacked `since_ts`; `DataPlaneStores` had no `indexing_event_store` field; `serve_full`'s `full()` didn't pass the store; `subscribe`'s replay segment only spliced the memory audit replay. task-43.1 closes all 4: `list_since(limit, since_ts)` (`WHERE ts_unix >= ?` when `since_ts>0`, mirroring `replay_events_from_audit`'s `ts<since_ts→skip`; `since_ts<=0` → no filter, byte-equiv `list()`) + `DataPlaneStores.indexing_event_store` field + `full()` 10th param (existing ctors get `None` byte-equiv) + `serve_full` clones the store into `DataPlaneStores` (write path keeps the original `Arc`, read path gets the clone — shared `Mutex<Connection>`) + `subscribe` splices indexing replay (`since_ts>0`: `list_since` + `indexing_rows_to_pb_events`, **after** audit replay, **before** live forward; store `None` / lock failure → `unwrap_or_default` empty, best-effort mirroring audit). So a `since_ts>0` reconnecting subscriber now receives the missed `indexing.progress`/`.cancelled`/`.error` events (the indexing counterpart of the task-26.2 audit replay). **Default byte-equiv** (`since_ts<=0` / `store=None`); **0 new dependency** / **0 network** / **0 schema migration** (reuses Phase 33 migration 0019) / **0 proto change**. **Honest-defer (ADR-013)**: this delivers unit-level splice + timing tests (`TEST-43.1.1`/`.2a`/`.2b`/`.2c`, lib 225→229); live daemon restart-then-replay e2e (real process + cross-restart dual-window assertion) is 🟡 `[SPEC-DEFER:phase-future.indexing-replay-daemon-e2e]`; `memory-actor-all-rpc` (not a small debt — Deprecate/SoftDelete need 7-layer + new migration, HardDelete needs audit-layer redesign) is honest-deferred to an independent phase (roadmap §3.17/§3.22 "schedule small, don't pad"). **ADR-048 → Accepted**; ADR-038 add-only Phase-43 Amendment (indexing-replay-e2e splice dimension fulfilled, live daemon e2e still deferred); ADR-014 — 34th activation.

详 `RELEASE_NOTES.md` v0.36.0 段 + [Phase 43 spec](docs/specs/phases/phase-43-governance-debt-cleanup-4.md) + [ADR-048](docs/decisions/adr-048-indexing-replay-splice.md)。

## What's new in v0.35.0

🏷️ **v0.35.0 chunk-source-type-filter** — turns the chunk-search `source_type` filter that Phase 32 (task-32.3 / ADR-037) honestly recorded as a *documented no-op* into a **real filter**. Grounding: `source_type` is **derivable from `file_path`** (`core/src/retriever/mod.rs classify_source_type`, mirroring `indexer::lang_hint_from_path`: extension → coarse bucket `code` / `doc` / `config` / `other`), so it needs **no storage and no chunks-schema migration** (chunks/files/provenance SQL_SCHEMA stays §5.3 FROZEN — a deterministic derivation equals a stored value). task-42.1 derives + **populates** `source_type` on every hit (filling the old `DEFAULT_SOURCE_TYPE=""` v0.1 schema gap) and post-filters `search()` BM25 (mirrors the language post-filter; empty `source_type` → no filtering → byte-equivalent); the v1 `server.rs:440-453` mapping was already wired, so `{"filters":{"source_type":["doc"]}}` works immediately over v1 gRPC/REST. task-42.2 forwards it to console-api: `console_data_plane` `SearchRequest` add-only `repeated string source_type = 9` (existing fields 1-8 frozen, ADR-015) + a `data_plane` post-filter (covers BM25/semantic/hybrid uniformly via the populated `h.source_type`) + Go `?source_type=` query/body union (mirrors `?semantic`/`?hybrid`) + grpcclient → `pb.source_type`; the console response `source_file_type` was already wired, so it shows the real derived value once populated. **Honest correction (ADR-013)**: `chunk-source-type-filter` and `chunk-agent-scope-filter` are **asymmetric** — `source_type` is derivable (delivered here); `agent_scope` is fundamentally a memory-layer concept (`memory_items` 0013 / `ListMemory` scope), chunks carry no agent dimension, so a real chunk agent_scope filter is **honest-deferred** (`[SPEC-DEFER:phase-future.chunk-agent-scope-filter]`), not faked. **0 new dependency** (`classify_source_type` is pure std) **/ 0 network / 0 schema migration / empty filter byte-equivalent**. **ADR-047 → Accepted**; ADR-037 add-only Phase-42 Amendment (source_type no-op superseded, agent_scope no-op reaffirmed); ADR-014 — 33rd activation.

详 `RELEASE_NOTES.md` v0.35.0 段 + [Phase 42 spec](docs/specs/phases/phase-42-chunk-source-type-filter.md) + [ADR-047](docs/decisions/adr-047-chunk-source-type-filter.md)。

## What's new in v0.34.0

🔤 **v0.34.0 tokenizer-default-on** — makes the deliberate product decision Phase 30 / ADR-035 §D3 honest-deferred (`[SPEC-DEFER:phase-future.tokenizer-default-on]`): the code/CJK-aware analyzer `code_cjk` (task-24.1, pure-std, 0-dep: camelCase/snake_case/dotted.path/kebab-case subword split + original-token-preserving + CJK bigram) flips from **opt-in** to the **production default for newly created collections**, so every user gets the recall uplift by default. This is the **first deliberate default-behavior change** (a new collection's `content` inverted-index terms go `TEXT` → `code_cjk`, **NOT byte-equivalent**), owned by **ADR-046** with three safety guarantees: (1) **existing collections are unaffected** — `open_with_tokenizer` reads the persisted `meta.json` schema for an existing index and ignores the flip, so old `TEXT` collections keep working; (2) **opt-out** back to legacy `TEXT` via `CONTEXTFORGE_TOKENIZER=default` (env) or config.toml `[retrieval] tokenizer = "default"`; (3) **no auto-migration** — existing collections upgrade to `code_cjk` only via the user-initiated `reindex_with_tokenizer`. Real measured recall delta over the current 16-question golden: before(default `TEXT`) recall@5/@10=0.8750 → after(`code_cjk`) recall@5/@10=**1.0000**, **delta +0.1250** (mrr +0.0625; consistent with Phase 30's `delta(seg−default)=+0.1250` — the historical `+0.0909` was Phase 24's original 11-question golden, ADR-013). **0 new dependency** (`code_cjk` is pure std; jieba `cjk_segmenter` stays feature-gated — Phase 30 measured jieba-vs-bigram delta=+0.0000, so the default flips to bigram `code_cjk`, not jieba) **/ 0 network**. **ADR-046 → Accepted**; ADR-029/035 add-only Phase-41 Amendments; ADR-014 — 32nd activation.

- **tokenizer-default-on** (task-41.1, #262): `core/src/server.rs` `resolve_tokenizer()` (reads `CONTEXTFORGE_TOKENIZER`: unset/"" → `code_cjk` flip / `"default"` → legacy `TEXT` opt-out / unknown·feature-off → stderr WARN + `code_cjk`, never silently `TEXT`) + the two production indexing call sites (`server.rs:141` `CoreService::index` + `jobs/index_session_backend.rs:151`) now `open_with_tokenizer(.., &resolve_tokenizer())`. `IndexSession::open` / `DEFAULT_TOKENIZER` (library entry + constant) unchanged. (TEST-41.1.1 env matrix / TEST-41.1.2 production binds code_cjk + opt-out TEXT + existing-collection safety; real recall delta +0.1250.)
- **tokenizer-config-bridge** (task-41.2, #263): Go `internal/config/config.go` add-only `RetrievalConfig{Tokenizer}` + `[retrieval]` section round-trip + `cmd/contextforge/main.go` `setTokenizerEnv` (mirrors `setVectorEnv`; env-wins; no `[retrieval]` → nothing exported → core defaults to `code_cjk`). Tokenizer is not a secret; Rust core 0 toml dep. (TEST-41.2.1 round-trip / TEST-41.2.2 env-bridge env-wins.)
- **closeout** (task-41.3): **ADR-046 tokenizer-default-on → Accepted** (per-D). **ADR-029/035** add-only Phase-41 Amendments (default-on dimension / D3 product decision fulfilled), NOT retro-editing their D-body (ADR-014 D5). **ADR-014 cross-validation gate — 32nd activation**. Smoke v31 [50/50] (`TestTask413`; REAL mode the camelCase subword `runner` of `JobRunner` hits via the `code_cjk` default — legacy `TEXT` would keep `jobrunner` single token and miss, a distinguishing assertion).

```bash
# tokenizer default flip + env opt-out: resolve_tokenizer env matrix + production binding + existing-collection safety
cargo test -p contextforge-core --lib test_41_1
# real recall delta over the current golden (default TEXT 0.8750 → code_cjk 1.0000, +0.1250)
cargo run -p contextforge-core --example phase24_tokenizer_recall
# Go [retrieval] tokenizer config + setTokenizerEnv (env-wins, no section → core default code_cjk)
go test ./internal/config/... -run TestTask412RetrievalConfig && go test ./cmd/contextforge/... -run TestSetTokenizerEnv
```

详 `RELEASE_NOTES.md` v0.34.0 段 + [Phase 41 spec](docs/specs/phases/phase-41-tokenizer-default-on.md) + [ADR-046](docs/decisions/adr-046-tokenizer-default-on.md)。
