...
**Upgrade / Rollback**：默认行为 / proto 既有字段 / 既有契约不变（既有 v0.6–v0.28 client + 索引/数据无需改动），**0 backend 改动、0 新代码依赖、0 network 默认构建**（ADR-004/008，qdrant-client 自 task-18.4 即 optional / qdrant_live_recall.rs env-gated 默认跳过）；改动均 test/CI-only（live recall harness + qdrant-recall service-container job），不改任何返回值/控制流/RPC 行为。linux/amd64（承 v0.21–v0.28；arm64 仍 deferred）；cosign keyless sign + SBOM + provenance（承 task-28.2）。Rollback：`git tag -d v0.29.0` + 删 Release / ghcr tag；与 v0.28.0 行为兼容（harness env-gated 默认不运行，CI job job-scoped 不影响默认构建）。tag SHA `<backfill: tag SHA>`（annotated tag obj `<backfill: annotated tag obj>`）/ release run id `<backfill: release run id>`（success）/ ghcr digest `<backfill: ghcr digest>`（ghcr.io/tajiaoyezi/contextforge-daemon:v0.29.0 + :latest，linux/amd64）/ cosign Rekor tlog `<backfill: cosign tlog sign>`（sign）·`<backfill: cosign tlog attest>`（attest）（post-tag-push 回填，ADR-012/013）。
