...
> 项目可在 `<adapter-path>` §Workflow Overrides 微调单一字段（如关闭 multi-reviewer / require-CI-gate）以适配实际团队规模。
>
> **任何 agent 进入本仓库时第一件事：读完本文件 + `<adapter-path>`**。
>
> 命名约定：**分支**一律含模块 name 全名（`feat/<phase>-<name>` / `feat/task-<X.Y>-<name>`，与 git 历史一致避免歧义）。**worktree 目录**：phase 级 `<repo>-wt-<phase-name>/`（主 agent 手动建）；task 级 `<repo>-wt-task-<X.Y>/`（`/s2v-implement` 步 4.B 自动建 — 用 X.Y 唯一标识，name 已在分支名，二者由 X.Y 关联）。
...
| # | 目录 | 分支 | 角色 | Owner | 启动门槛 |
|---|---|---|---|---|---|
| 0 | `<repo>/` (主 repo) | `main` | 协调中心 / merge PR / phase smoke gate | **仅主 agent** | — |
| 1 | `<repo>-wt-<phase-1-name>/` | `feat/<phase-1-name>` | Phase 1 任务工作区 | 实施 Phase 1 的 agent | ✅ 立即（无前置）|
| 2 | `<repo>-wt-<phase-2-name>/` | `feat/<phase-2-name>` | Phase 2 任务工作区 | 实施 Phase 2 的 agent | ⏳ 等 Phase 1 merge |
| ... | ... | ... | ... | ... | ... |
...
新增 worktree 命名规则：phase 级 `<repo>-wt-<phase-name>/` + 分支 `feat/<phase-name>`；task 级 `<repo>-wt-task-<X.Y>/` + 分支 `feat/task-<X.Y>-<name>`（worktree 目录用 X.Y、分支含 name — 与 `/s2v-implement` 步 4.B 落盘一致）。
...
```bash
# 0. 验证不在主 repo
test "$(basename "$PWD")" != "<repo>" || { echo "ERROR: 不要在主 repo"; exit 1; }
...
#         '../<repo>-wt-task-<X.Y>'`（步 4.B 建的 worktree 仍占用该分支）。
git worktree remove "../<repo>-wt-task-<X.Y>" 2>/dev/null \
  || { [ -e "../<repo>-wt-task-<X.Y>" ] && echo "⚠️ Gate 0: worktree 仍存在但 remove 失败（可能有未提交改动）— 处理后 git worktree remove --force 再重跑 gate" >&2; true; }
...
ADAPTER="<adapter-path>"        # agent 替换为实际 adapter（如 docs/s2v-adapter.md）
...
git worktree add "../<repo>-wt-<phase-name>" "feat/<phase-name>"
# task 级 worktree 由 /s2v-implement 步 4.B 自动建（../<repo>-wt-task-<X.Y> + feat/task-<X.Y>-<name>）— 主 agent 通常无需手动建
```
...
> 通用约束：所有外部 agent **严禁进入主 repo `<repo>/`**，**严禁直接 push main**，**严禁 merge 自己的 PR**。

### Codex
- 默认读取 AGENTS.md
- 启动后必读：`<adapter-path>` + 派工 prompt 指定的 task spec + 该 spec §5.1 Required Reading
