...
- **task-42.3** closeout：smoke v31→v32[51/51]（REAL source_type 真实过滤端到端：.rs+.md 混合 fixture / `?source_type=doc` 仅返 doc / `?source_type=code` 仅返 code / 空 filter 返全部，distinguishing）+ TestTask423（no [37/37]..[50/50] 回归）+ release docs + ADR-047 ratify + ADR-037 add-only Phase-42 Amendment（source_type no-op supersede / agent_scope no-op 保持）+ roadmap §3.24/§4 + adapter。🟢。
