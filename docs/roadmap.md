...
**v0.36.0 推进记录（已落地 2026-07-01，add-only）**：§3.25 全 task 合入 master，ADR-048 据 D1-D4 真实 ratify（Proposed → Accepted；D4 live daemon e2e 🟡 honest-defer）：
- **task-43.1**（PR #276，master @ `2c98cc2`）indexing-replay-splice → ✅：`core/src/data_plane/indexing_events.rs` add `pub fn list_since(&self, limit, since_ts)`（since_ts>0 时 `WHERE (?1=0 OR ts_unix>=?1) ORDER BY id ASC LIMIT ?2` 单 SQL 守护统一 params，镜像 `replay_events_from_audit` 的 `ts<since_ts→skip`；since_ts<=0 不过滤 byte-equiv `list()`；既有 `list(limit)` 不动）+ TEST-43.1.1（list_since 时序过滤 + id ASC + since_ts<=0 byte-equiv + limit clamp + re-open idempotent）；`core/src/data_plane/mod.rs` `DataPlaneStores` add `pub indexing_event_store: Option<Arc<SqliteIndexingEventStore>>` 字段 + `full()` 第 10 参数（5 既有 constructor `new`/`with_eval`/`with_memory`/`with_runner`/`with_runner_and_bus` 补 None byte-equiv）+ `memory.rs` test struct literal 补字段；`core/src/server.rs` serve_full `IndexSessionBackend::with_event_bus_and_indexing_store(event_bus, indexing_event_store.clone())`（写路径持 clone）+ `DataPlaneStores::full(..., Some(indexing_event_store))`（读路径 subscribe 可达，原件 move 进，共享 `Mutex<Connection>`）；`core/src/data_plane/events.rs` `subscribe` replay 段（since_ts>0 分支）splice indexing replay（`self.stores.indexing_event_store.as_ref().and_then(|s| s.list_since(1000, req.since_ts).ok()).map(|rows| indexing_rows_to_pb_events(&rows)).unwrap_or_default()`，audit replay 后、live forward spawn 前；store None/lock 失败 unwrap_or_default 空 best-effort 镜像 audit）+ helper `server_with_replay_sources` + TEST-43.1.2a（subscribe splice 顺序 indexing→audit→live）+ TEST-43.1.2b（since_ts<=0 无 replay byte-equiv timeout 守护）+ TEST-43.1.2c（store=None 仅 audit replay 无 indexing）。lib 225→229 全绿 + clippy 0 warning + workspace 全绿。
- **task-43.3** closeout → ✅：smoke v32→v33 `[52/52]`（indexing-replay-splice 可达断言 / 不可达诚实归因 unit TEST-43.1.2a/b/c；REAL-mode subscribe-stream e2e 🟡 honest-defer）+ `TestTask433`（no-regression [37/37]..[51/51]）+ v0.36.0 release docs（evidence/artifacts/README/RELEASE_NOTES）+ ADR-048 per-D ratify（D1-D3 unit 🟢 / D4 默认 byte-equiv 🟢 + live daemon e2e 🟡 honest-defer）+ ADR-038 add-only Phase-43 Amendment（indexing-replay-e2e splice 维度兑现，live daemon e2e 续延后，不溯改 D-body D5）+ ADR-031/021/004/008/013/014 守线引用 + roadmap/adapter。

**关键诚实定性（ADR-013）**：本 phase 交付 splice **拼接**（接进 live subscribe 路径 + since_ts 时序对齐 audit + 默认 byte-equiv），🟢 纯本地单测守护时序 + 拼接 + 退化 byte-equiv（lib 225→229）；live daemon restart-then-replay 端到端 e2e（真起进程 + 跨 restart 双窗口断言）须 running daemon（须 console 跨进程）→ 🟡 honest-defer `[SPEC-DEFER:phase-future.indexing-replay-daemon-e2e]` 不预填、不夸大 unit splice 为 live e2e 已验；memory-actor-all-rpc（Deprecate/SoftDelete 7 层 + 新 schema migration / HardDelete 须 audit 层重设计 = 非小债）据实延后留独立 phase（roadmap §3.17/§3.22 "据实排小不凑数"，不强行扩面）。**新增 backlog 条目（add-only）**：`indexing-replay-daemon-e2e`（live daemon restart-then-replay 端到端 e2e，须 running daemon + 跨 restart 双窗口）/ `memory-actor-all-rpc`（四 memory RPC 的 actor 透传，Deprecate/SoftDelete 7 层 + 新 migration / HardDelete 须 audit 重设计，独立 phase）。**0 新 dep + 0 network + 0 schema migration（复用 Phase 33 migration 0019）+ 0 proto 改动 + 默认 byte-equiv（since_ts<=0 / store=None）**；core lib 229 全绿 + go test（含 TestTask433）+ clippy + go vet + gofmt clean + CI spec-lint 全绿。真实 v0.36.0 tag/run/digest/tlog 经用户授权 push（ADR-012），post-tag-push 回填（ADR-013 不预填）。
