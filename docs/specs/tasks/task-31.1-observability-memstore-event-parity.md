...
- [ ] AC1（memstore-event-emit Go fallback parity）: Go fallback `MemMemoryStore` 的 `Pin` / `Deprecate` / `SoftDelete` / `Unpin` / `HardDelete` 在成功变更后向 fallback ring 发对应 `memory.pin` / `memory.deprecate` / `memory.soft_delete` / `memory.unpin`（共 `memory.pin` payload `op=unpin`）/ `memory.hard_delete` 事件；event_type 命名与 Rust `audit_op_to_event_type` 对齐；五写操作返回 / 错误契约不变（🟢 deterministic CI-verifiable）— verified by TEST-31.1.1
- [ ] AC2（event-bus partition/capacity verify-only + roadmap add-only 更正）: `core/src/data_plane/events.rs` 既有 `TEST-26.3.1a/b/c` 保持常绿（经核 Phase 26 / ADR-031 D5 已交付，不重复实现）；`docs/roadmap.md` §4 add-only 更正记录 event-bus-partition / event-bus-capacity 经核 Phase 26 已交付（不改 Rust 源码）（🟢 deterministic CI-verifiable）— verified by TEST-31.1.2
- [ ] AC3（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by TEST-31.1.3

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-31.1.1 | Go fallback `MemMemoryStore` Pin/Deprecate/SoftDelete/Unpin/HardDelete 发 `memory.*` 入 fallback ring（单测断言 fallback 模式下 Pin 后 ring 增长 + 五写操作各发对应 event_type + 返回契约不变） | `internal/consoleapi/memstore.go` + `internal/consoleapi/*_test.go` | Planned |
| TEST-31.1.2 | event-bus partition/capacity verify-only（`TEST-26.3.1a/b/c` 常绿）+ roadmap §4 add-only 更正记录（经核 Phase 26 / ADR-031 D5 已交付，不重复实现） | `core/src/data_plane/events.rs`（verify-only）+ `docs/roadmap.md` | Planned |
| TEST-31.1.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威） | `scripts/spec_drift_lint.sh` | Planned |
...
- **Status**: Draft（待实施）。
- **计划改动文件**：
  - `internal/consoleapi/memstore.go`——`MemMemoryStore` 的 `Pin` / `Deprecate` / `SoftDelete` / `Unpin` / `HardDelete` 成功变更后调 fallback 事件发射，发 `memory.pin` / `memory.deprecate` / `memory.soft_delete` / `memory.unpin`（共 `memory.pin`）/ `memory.hard_delete`（event_type 对齐 Rust）。
  - `internal/consoleapi/*_test.go`——新增单测断言 fallback 模式下五写操作发对应 event_type + ring 增长 + 返回契约不变（含 `-race`）。
  - `docs/roadmap.md` §4——add-only 更正行：event-bus-partition / event-bus-capacity 经核 Phase 26 / ADR-031 D5 已交付（不改既有正文）。
  - `core/src/data_plane/events.rs` / `memory.rs`——**不改**（verify-only / 已交付参照）。
- **§9 Verification 计划** (will record real evidence at impl)：
  - AC1：`go test ./internal/consoleapi/... -race` + `go test ./...` 真实结果待实测回填。
  - AC2：`cargo test` `TEST-26.3.1a/b/c` 常绿 + roadmap §4 add-only 更正落地，真实结果待实测回填。
  - AC3：`scripts/spec_drift_lint.sh --touched origin/master` 触及行 0 未标注命中，CI spec-lint 权威，真实结果待实测回填。
