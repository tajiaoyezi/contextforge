...
- [ ] AC1（smoke v20 step 39）: smoke banner v19→v20 + step 39（`[39/39]` 默认构建 init baseline + 默认 tokenization 不变 + `cjk-segmenter` feature-gated 状态）+ 新 `Test` 断言（含无回归既有 `[37/37]` / `[38/38]`，denominator 不溯改）— verified by TEST-30.3.1
- [ ] AC2（v0.23.0 closeout bundle）: v0.23.0 release docs（evidence/artifacts angle-bracket backfill 待回填 + README + RELEASE_NOTES）+ ADR-035 据 D1-D5 真实 ratify Accepted（per-D 如实，重词典 / 大语料受阻维度部分 ratify）+ ADR-029 add-only Amendment + phase-30 §6 AC1-4 `[x]` + Status Done + adapter 闭合 + feature 已存在 — verified by TEST-30.3.2
- [ ] AC3（ADR-014 D2 lint）: bash scripts/spec_drift_lint.sh --touched origin/master PR 触及行 0 未标注命中 — verified by TEST-30.3.3

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-30.3.1 | smoke v20 step 39（`[39/39]` + 默认 tokenization baseline 标记 + 无回归既有 denominator）+ `bash -n` 过 + go test 新 Test 过 | `scripts/console_smoke.sh` + `internal/cli/smoke_syntax_test.go` | Planned |
| TEST-30.3.2 | release docs + ADR-035 ratify Accepted（per-D 如实，受阻维度部分）+ ADR-029 add-only Amendment + phase-30 §6 闭合 + adapter + feature bundle | release/ADR/phase/adapter/feature | Planned |
| TEST-30.3.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威） | `scripts/spec_drift_lint.sh` | Planned |
