...
- [ ] AC2（in-process 选择矩阵 wiring 🟢 + 矩阵 cell honest-defer 🟡）: 补 arm 后选择矩阵 wiring 完整——`select_vector_backend("sqlite-vec", dim)`（feat on）返回 `name()=="sqlite-vec"` 后端（wiring 🟢 单测断言）；矩阵 recall@k / latency CELL 须本机 `x86_64-pc-windows-msvc` `--features vector-sqlite` 真实 build + 真实语料跑出 → 🟡 `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`（真实数值真实跑出才回填，本 task **不预填**，ADR-013）；默认 build 0 vector-dep（ADR-004）+ 0 新 dep（ADR-008） — verified by TEST-32.2.2
- [ ] AC3（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by TEST-32.2.3（LAST）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-32.2.1 | factory `"sqlite-vec"` arm 双半 gating（镜像 TEST-29.1.2）：feat off → honest Err（msg 含 `sqlite-vec` + `vector-sqlite`，不回落 BruteForce）；feat on（`--features vector-sqlite`）→ 后端 `name()=="sqlite-vec"`；既有 arm 不变 | `core/src/retriever/vector/factory.rs`（同源 test） | Planned |
| TEST-32.2.2 | in-process 选择矩阵 wiring 🟢：`select_vector_backend("sqlite-vec", dim)`（feat on）选中正确后端（`name()` 断言）+ 默认 build 0 vector-dep；矩阵 recall/latency CELL 🟡 `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`（须本机 MSVC feature build 真实跑出才回填，不预填） | `core/src/retriever/vector/factory.rs`（同源 test） | Planned |
| TEST-32.2.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威）— LAST | `scripts/spec_drift_lint.sh` | Planned |
...
- AC1：`cargo test -p contextforge-core retriever::vector::factory`（feat-off 半 + 既有 arm 不变，默认 build 🟢）+ `cargo test -p contextforge-core --features vector-sqlite retriever::vector::factory`（feat-on 半，本机 MSVC 🟢）——feat off → honest Err（msg 含 `sqlite-vec` + `vector-sqlite`，不回落 BruteForce）；feat on → 后端 `name()=="sqlite-vec"`。真实通过结果待实施回填（ADR-013，不预填）。
- AC2：in-process 选择矩阵 wiring 🟢（`select_vector_backend("sqlite-vec", dim)` feat on 选中正确后端，`name()` 断言）+ 默认 build 0 vector-dep（ADR-004）+ 0 新 dep（ADR-008）。矩阵 recall@k / latency CELL 🟡 `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`——须本机 `x86_64-pc-windows-msvc` `--features vector-sqlite` 真实 build + 真实语料跑出，真实数值真实跑出才回填，本 task body **不预填任何数字**（ADR-013）。
- AC3：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- 0 新 dep / 0 新 feature（`sqlite-vec` 既 optional 在 `Cargo.toml:82` + `vector-sqlite` 既声明 `:127`）/ 默认行为不变（既有 TEST-29.1.* 全绿）/ 既有契约不变（`select_vector_backend` 签名不变）真实结果待实施回填。

**计划改动文件**（at impl）：
- `core/src/retriever/vector/factory.rs`——`match name` 加 `"sqlite-vec"` arm（镜像 qdrant `:40-51` / lancedb `:52-63` 双半 cfg）+ rustdoc（`:18-30`）add-only 列出该 arm + `#[cfg(test)] mod tests` 加 TEST-32.2.1（feat-off 半 + feat-on 半，镜像既有 `:84-122`）+ TEST-32.2.2（wiring）。
- 矩阵 recall/latency cell：honest-defer `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`，真实跑出才回填（非本 task body 数字）。
- ADR-037 据真实测试 ratify（含 D2 / D4 据实延后 cell `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`）+ ADR-034 sqlite-vec arm 补全 factory 的 add-only Amendment 落点在 task-32.4 closeout（非本 task body）。
