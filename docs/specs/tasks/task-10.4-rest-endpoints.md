...
- **新增 `internal/consoleapi/router.go`**：
  ...
  - `type Deps struct { WorkspaceClient WorkspaceClient; JobClient JobClient; SearchClient SearchClient; EventsClient EventsClient; AuthToken string }` — 注入式依赖（unit test 可 mock）
...
- [ ] AC2：每个 endpoint handler 实现 + 单元测试（mock Deps）覆盖 happy path + 错误码 case — **verified by unit-test step `go test ./internal/consoleapi/... -run 'TestHandle.*'`**
