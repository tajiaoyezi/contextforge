...
**Pending → Ready trigger**: 见 [ADR-022 D5](../../decisions/adr-022-memory-is-pinned-field-amendment.md#d5--phase-17-pending--ready-trigger) — 用户人工转发 Console PR merge SHA → ContextForge 主 agent 验证 Console master HEAD 含 IsPinned 字段 → Status `Pending → Ready` → 启动实施
...
- [ ] AC1: SqliteMemoryStore `set_pinned(memory_id, true)` 后 `get_by_id(memory_id)` 返 `is_pinned: true`；`set_pinned(memory_id, false)` 后返 `false` — **verified by `core/src/memory/store.rs::tests::test_sqlite_set_pinned_true_persists_get` + `test_sqlite_set_pinned_false_reverses` PASS**
- [ ] AC2: MemMemoryStore fallback (`CONSOLE_API_FALLBACK_INMEM=1` 显式 opt-in) Pin(id, true) 后 Get(id).IsPinned = true；Pin(id, false) 后 = false — **verified by `internal/consoleapi/memstore_test.go::TestMemMemoryStore_Pin_TogglesIsPinned` PASS**
- [ ] AC3: SqliteMemoryStore.list / MemMemoryStore.List 返字段 — list 响应每项 MemoryItem 含 `is_pinned: bool` — **verified by `core/src/memory/store.rs::tests::test_list_returns_is_pinned_column` + `internal/consoleapi/memstore_test.go::TestMemMemoryStore_List_ReturnsIsPinned` PASS**
- [ ] AC4: Migration 幂等 — fresh install 应用；既有 v0.9 DB 升级后 `is_pinned` 列存在 + 既有数据全 backfill `0`；重复 daemon 启动不报错 — **verified by `core/tests/memory_persist_integration.rs::test_is_pinned_migration_applies_idempotently` PASS**
- [ ] AC5: contractv1 forward/backward compat — Console v0.7-v0.9 (pre-amend) client 解析 v0.10 response 不破坏；v0.10+ client 解析 v0.9 response `is_pinned` 默认 `false` — **verified by `internal/contractv1/contractv1_test.go::TestMemoryItemForwardBackwardCompat` PASS**
- [ ] AC6: `Pin RPC` 端到端 — `POST /v1/memory/{id}/pin {"pin":true}` (X-Confirm: yes) → 204 No Content + audit_log 记录 + EventBus emit `memory.pin` event + SqliteMemoryStore.is_pinned 列 = 1；`GET /v1/memory/{id}` 返 `is_pinned: true` — **verified by smoke v8 Step 28 (daemon-level full roundtrip + restart) PASS**
- [ ] AC7: 既有 22-endpoint conformance 不退化（contract v1 不 bump；仅 MemoryItem 字段 add-only）；`cargo test --workspace` 121+ lib + 17 integration 不退化；`go test ./...` 22 packages 不退化 — **verified by 本 PR 跑 cargo + go + conformance 实测**
- [ ] AC8: ADR-014 D2 lint — `scripts/spec_drift_lint.sh --touched origin/master` 0 unannotated hits（本 task spec 内所有 anti-pattern 已标 [SPEC-DEFER] / [SPEC-OWNER]）— **verified by 本 PR body D2 输出段**

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-17.1.1 | SqliteMemoryStore::set_pinned true 持久化 | `core/src/memory/store.rs::tests::test_sqlite_set_pinned_true_persists_get` | Pending |
| TEST-17.1.2 | SqliteMemoryStore::set_pinned false 反转 | `core/src/memory/store.rs::tests::test_sqlite_set_pinned_false_reverses` | Pending |
| TEST-17.1.3 | SqliteMemoryStore::list 返 is_pinned | `core/src/memory/store.rs::tests::test_list_returns_is_pinned_column` | Pending |
| TEST-17.1.4 | MemMemoryStore.Pin 切换 IsPinned | `internal/consoleapi/memstore_test.go::TestMemMemoryStore_Pin_TogglesIsPinned` | Pending |
| TEST-17.1.5 | MemMemoryStore.List 返 IsPinned | `internal/consoleapi/memstore_test.go::TestMemMemoryStore_List_ReturnsIsPinned` | Pending |
| TEST-17.1.6 | Migration 幂等 + 升级 backfill | `core/tests/memory_persist_integration.rs::test_is_pinned_migration_applies_idempotently` | Pending |
| TEST-17.1.7 | contractv1 forward/backward compat | `internal/contractv1/contractv1_test.go::TestMemoryItemForwardBackwardCompat` | Pending |
| SCEN-17.1.1 | Pin RPC + GET + restart full roundtrip | `scripts/console_smoke.sh` v8 Step 28 | Pending |
...
- **ADR-021 EventBus emit 路径不破坏**：task-15.2 ship 的 `emit_audit_and_event` 路径必须保留；本 task 仅在前 / 后追加 `store.set_pinned` 调用，不动 audit + EventBus 顺序
...
## 10. Completion Notes (待 Ready 实施后回填，s2v 6 项标准)

- [ ] commit SHA / PR # / merge SHA
- [ ] cargo test --workspace 实测输出
- [ ] go test ./... 实测输出
- [ ] smoke v8 CONSOLE_REAL_SMOKE_EXIT=0 实测
- [ ] ADR-014 D2 lint 0 unannotated hits 实测
- [ ] ADR-022 Status Proposed → Accepted 时间 + closeout PR #
