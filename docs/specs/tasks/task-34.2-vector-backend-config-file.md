...
- [x] **AC1**（Go config `[vector]` 段 TOML round-trip + `setVectorEnv` env-wins 🟢）: `VectorConfig{Backend,Dim}` + `Config.Vector` add-only 字段；`encodeTOML`/`decodeTOML` 对称编解码 `[vector]` 段（含 / 不含段双向 round-trip 恒等），既有 `[collections]`/`[remote]`/`[embedding]` 段不受影响（TEST-1.2.* 不退化）；`setVectorEnv`：段存在（backend/dim 非零）⇒ `CONTEXTFORGE_VECTOR_BACKEND`/`CONTEXTFORGE_VECTOR_DIM` 经 `os.Setenv` 导出；env 已显式设 ⇒ 不覆盖（env-wins）；无段（`"",0`）⇒ 不导出；restore 闭包恢复旧值；**Rust core 0 toml dep** — verified by **TEST-34.2.1**（config round-trip）+ **TEST-34.2.2**（setVectorEnv env-wins）
- [x] **AC2**（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by **TEST-34.2.3**（= LAST）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-34.2.1 | Go config `[vector]` 段 TOML round-trip：含 `[vector]`（`backend`/`dim`）Save→Load 恒等 + 不含 `[vector]` 段 Load 得 zero value（`Backend ""`/`Dim 0`）+ 既有 `[collections]`/`[remote]`/`[embedding]` 段编解码不受影响（不退化） | `internal/config/config_test.go`（同包 test） | Done |
| TEST-34.2.2 | `setVectorEnv` env-wins：段存在（backend/dim 非零）⇒ `CONTEXTFORGE_VECTOR_BACKEND`/`CONTEXTFORGE_VECTOR_DIM` 经 `os.Setenv` 导出（spawned core daemon 经 env 路径拾取）；env 已显式设 ⇒ 不被 config 覆盖（env-wins）；无段（`"",0`）⇒ 不导出（unset ⇒ BruteForce 字节等价）；restore 闭包恢复旧值 / unset | `cmd/contextforge/main_test.go`（同包 test） | Done |
| TEST-34.2.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威）（= LAST） | `scripts/spec_drift_lint.sh` | Done |
...
- AC1：`go test ./internal/config/ -run TestTask342` PASS —— Go config `[vector]` 段（`backend`/`dim`）Save/Load round-trip 保真 + 既有 `[remote]`/`[embedding]`/`[[collections]]` 段不退化 + 不含段（legacy）Load 得 zero value（TEST-34.2.1）；`go test ./cmd/contextforge/ -run TestSetVectorEnv` PASS（3 子例：`[vector]` 存在导出 + restore unset / 显式 env 不被覆盖 env-wins / 空 `[vector]` 不导出 byte-equivalent）（TEST-34.2.2）。`setVectorEnv` 已接线 doServe + doMCP 两 daemon-up 路径（best-effort `config.Load`，load 失败非致命）。
- Rust core 0 toml dep：`grep -c '^toml ' core/Cargo.toml` = 0（依赖面零变化，未引 toml reader 到 Rust）。
- AC2：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- 不退化：`go test ./...` 全 PASS（exit 0）；`go vet ./cmd/... ./internal/config/...` clean；无 Rust 改动（cargo 不受影响）。
- 0 新 dep / 默认行为不变（无 `[vector]` 段 = 不导出 = unset = BruteForce 字节等价，ADR-004） / 既有契约不变 / env-wins 向后兼容（显式 env 覆盖 config file）。

**实际改动文件**：
- `internal/config/config.go`——加 `VectorConfig{Backend; Dim}` + `Config.Vector VectorConfig`（与 `Embedding` 并列）；`encodeTOML` 加 `[vector]` 段输出（镜像 `[embedding]`）；`decodeTOML` 加 `case line == "[vector]"` + `assignVector`（镜像 `assignEmbedding`）。
- `cmd/contextforge/main.go`——加 `setVectorEnv(dataDir string) func()`（best-effort `config.Load(dataDir)` → 逐变量 `os.LookupEnv` env-wins 守卫 + `os.Setenv` + restore 闭包，镜像 `setDataDirEnv`）；接线 doServe（`daemon.Start` 前）+ doMCP（既有 `setDataDirEnv` 后）；import `internal/config` + `strconv`。
