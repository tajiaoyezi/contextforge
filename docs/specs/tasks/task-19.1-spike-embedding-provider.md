...
在 `core/src/embedding/` 落地 embedding 抽象层：`EmbeddingProvider` trait（`core/src/embedding/{mod,traits}.rs`）+ `DeterministicEmbeddingProvider`（`core/src/embedding/deterministic.rs`，hash/seed 派生，无模型 dep，默认构建启用，供 CI/smoke/test）+ 选定的 real provider（`core/src/embedding/<chosen>.rs`，`embedding-<chosen>` feature-gated，模型 lazy load）。spike 评估 3 路候选（fastembed-rs / candle / ort）在 Linux + Windows MSVC 的构建/运行/模型获取并选定一路，evidence 落 `docs/spikes/phase-19-embedding-{candidates,<chosen>}.md`。同源 `mod tests` ≥3 unit test（deterministic 确定性 + dim 一致 + trait 契约）。默认 `cargo test --workspace` + `go test ./...` 不退化；D2 lint 触及行 0 未标注命中。

## 3. Scope

### In Scope

- **新建 `core/src/embedding/mod.rs`** — 模块根：`pub mod traits; pub mod deterministic;` + cfg-gated `#[cfg(feature = "embedding-<chosen>")] pub mod <chosen>;` + re-export（`pub use traits::EmbeddingProvider; pub use deterministic::DeterministicEmbeddingProvider;` + cfg-gated real provider）+ `#[cfg(test)] mod tests;`。
...
- **新建 `core/src/embedding/<chosen>.rs`** — 选定的 real provider（fastembed/candle/ort 之一，依 §spike 结果定 `<chosen>`）：
  - 实现 `EmbeddingProvider`；模型 lazy load（首次 `embed` 触发 init，避免构造即下载）
  - `embedding-<chosen>` feature gate；默认构建不引入该 dep（cfg-gated）
...
- **修改 `core/Cargo.toml`** — `<chosen> = { version = "...", optional = true }` real-provider dep + `embedding-<chosen> = ["dep:<chosen>", ...]` feature（默认不启用，承 vector-* feature pattern）。
...
- **新建 `docs/spikes/phase-19-embedding-<chosen>.md`** — 选定 provider 的真实 embed 样例 evidence（输入文本 → 输出向量维度/范数/前若干分量、模型来源、首次 lazy load 行为、平台构建实证）。
...
- **real provider（`<chosen>`）**：cfg-gated 成员，dev / Linux 跑真实 embedding（喂 task-19.5 真实召回）。
...
// <chosen>.rs — feature-gated real provider（示意，<chosen> 依 spike 选定）
// use <chosen_crate>::{...};   // 经 embedding-<chosen> feature 引入，lazy model init
```
...
- **feature gate**：`embedding-<chosen>` 默认关闭；默认 `cargo build` 不编译 `<chosen>.rs`、不引入 `<chosen>` dep（cfg-gated，承 vector-sqlite/vector-hnsw pattern）。
...
- [ ] **AC1**: `EmbeddingProvider` trait（`embed` + `dim` + `name`）+ `EmbeddingError`（`#[non_exhaustive]`）落 `core/src/embedding/{mod,traits}.rs`；`Arc<dyn EmbeddingProvider>` 对象安全；默认 `cargo build -p contextforge-core` exit 0 且不引入 real-provider dep（cfg-gated）— verified by **TEST-19.1.1**（trait 对象安全 + 默认 build 无 `<chosen>` 编译）
- [ ] **AC2**: `DeterministicEmbeddingProvider` 确定性 — 同 text 两次 `embed` 逐分量相等，无模型 / 无网络 / 无新 dep，默认构建启用 — verified by **TEST-19.1.2**（确定性单测 PASS）
- [ ] **AC3**: dim 一致 — `embed` 输出每向量 `.len() == dim()`，多文本批量恒定，单位范数 invariant — verified by **TEST-19.1.3**（dim 一致 + 范数单测 PASS）
- [ ] **AC4**: 候选评估 evidence — `docs/spikes/phase-19-embedding-candidates.md` 记 3 路（fastembed-rs/candle/ort）Linux + Windows MSVC 构建/模型获取/API/选定理由（非伪造，stop-condition 据实记）；`docs/spikes/phase-19-embedding-<chosen>.md` 记选定 provider 真实 embed 样例或实测受阻凭据 — verified by **TEST-19.1.4**（两 evidence 文档存在 + 内容实证）
- [ ] **AC5**: 既有不退化 — 默认 `cargo test --workspace` 全 PASS（embedding-<chosen> 默认不启用，gated path 不入默认编译）；`go test ./...` 全 PASS — verified by **TEST-19.1.5**（`cargo test --workspace` 0 failed + `go test ./...` 0 failed）+ §10 实测
- [ ] **AC6**: ADR-014 D2 lint — `bash scripts/spec_drift_lint.sh --touched master` PR 触及行 0 未标注命中 — verified by **TEST-19.1.6**（§10 记录的 D2 lint 实跑输出）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-19.1.1 | trait 对象安全 + 默认 build 无 real-provider dep | `core/src/embedding/tests.rs` + `cargo build -p contextforge-core` | Pending |
| TEST-19.1.2 | deterministic 确定性（同 text 逐分量相等） | `core/src/embedding/tests.rs` | Pending |
| TEST-19.1.3 | dim 一致 + 单位范数 invariant | `core/src/embedding/tests.rs` | Pending |
| TEST-19.1.4 | 候选评估 + 选定 provider evidence 实证 | `docs/spikes/phase-19-embedding-{candidates,<chosen>}.md` | Pending |
| TEST-19.1.5 | 默认 cargo test --workspace + go test ./... 0 failed | 全 workspace | Pending |
| TEST-19.1.6 | D2 lint --touched master 0 未标注命中 | `scripts/spec_drift_lint.sh` | Pending |
...
- **R1（高）real provider 平台/模型门槛**：fastembed(ort)/candle native runtime + 模型下载在 Windows MSVC / CI 受阻（承 phase-19 §7 R1，类比 phase-18 sqlite-vec MSVC 受阻凭据）。
  - **缓解**：deterministic 缺省 provider 兜底（无模型 / 无网络 / 默认构建 0 新 dep，供 CI/smoke/test/wiring）；real provider `embedding-<chosen>` feature 默认关闭。stop-condition（ADR-013）：两平台均不可构建 → deterministic 续跑 wiring/smoke（诚实标注），real-model 召回 + ADR ratify 据实测延后 [SPEC-OWNER:phase-future.embedding-provider-full]。
...
cargo test --workspace        # 默认 feature，embedding-<chosen> gated 不入编译

# real provider（Linux dev，<chosen> 依 spike 选定）：feature build + 真实 embed 探针
cargo build -p contextforge-core --features embedding-<chosen>
cargo test -p contextforge-core --features embedding-<chosen> -- embedding
...
- **完成日期**：（实现后填）
- **改动文件**：`core/src/embedding/mod.rs`（新增）、`core/src/embedding/traits.rs`（新增 EmbeddingProvider + EmbeddingError）、`core/src/embedding/deterministic.rs`（新增 DeterministicEmbeddingProvider）、`core/src/embedding/<chosen>.rs`（新增 real provider，cfg-gated）、`core/src/embedding/tests.rs`（新增 ≥3 unit test）、`core/src/lib.rs`（`pub mod embedding;`）、`core/Cargo.toml`（`<chosen>` optional dep + `embedding-<chosen>` feature）、`docs/spikes/phase-19-embedding-candidates.md`（新增候选评估）、`docs/spikes/phase-19-embedding-<chosen>.md`（新增选定 evidence）、`docs/s2v-adapter.md`（19.1 行登记）—（实现后据实补 `<chosen>`）
- **commit 列表**：（实现后填）见本 task PR（分支 `feat/task-19.1-spike-embedding-provider`）；合入后以 merge commit 为准
- **§9 Verification 结果**：（实现后填）见 PR 描述与 `docs/spikes/phase-19-embedding-{candidates,<chosen>}.md`（Linux + Windows MSVC 构建实证 + deterministic 单测 + 默认 `cargo test --workspace` / `go test ./...` 0 failed + D2 lint 0 hits）
- **剩余风险 / 未做项**：（实现后填）default backend wiring 见 [SPEC-OWNER:task-19.2-default-backend-wiring]；真实 dogfood 召回见 [SPEC-OWNER:task-19.5-real-recall-eval]；ADR-023 ratify 见 [SPEC-OWNER:task-19.6-adr-023-ratify]；real provider 两平台受阻 stop-condition 触发与否据 §spike 实测，real-model 召回闭环 [SPEC-OWNER:phase-future.embedding-provider-full]；remote provider 见 [SPEC-DEFER:phase-future.embedding-provider-remote]
- **下游 task 影响**：（实现后填）task-19.2（消费 `EmbeddingProvider` 接 index/query 热路径）/ task-19.5（real provider 喂 dogfood 真实 `SemanticRecall@K`）/ task-19.3（`embedding_provider` provenance 字段源自本 trait `name()`）
