...
**§9 Verification 实证（本机真实跑，全 PASS）**：
- AC1：`go test ./internal/config/...`（TEST-37.2.1 `TestTask372RemoteModelConfig`：`[remote] model` Save/Load round-trip 保真 + 既有 `Enabled`/`Provider`/`Endpoint` + `[embedding]`/`[vector]`/`[[collections]]` 段不退化 + 不含 `model` 行 legacy Load 得 `Model=""`）→ **ok**；`go test ./cmd/contextforge/...`（TEST-37.2.2 `TestSetRemoteEnv`：字段非空 → 导出 `_ENDPOINT`/`_MODEL`/`_PROVIDER` + restore unset / 显式 env 不被覆盖（env-wins）/ 空 `[remote]` 不导出 / **`CONTEXTFORGE_REMOTE_API_KEY` 永不被 `setRemoteEnv` 导出** 安全红线）→ **ok**。
- Rust core 0 toml dep：`grep -c '^toml' core/Cargo.toml` = **0**（依赖面零变化，`factory.rs` 不改，兑现 `factory.rs:52` follow-up 注）。
- 不退化：`go build ./...` clean + `go vet ./internal/config/... ./cmd/contextforge/...` clean + 两包 `go test` ok；`gofmt` 改动文件为纯 CRLF（LF 规范化后 gofmt-clean，CI 在 LF 通过）。
