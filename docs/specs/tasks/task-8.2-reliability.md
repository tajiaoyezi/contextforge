...
- [x] **AC1** (PRD §Implementation Phases Phase 8 Exit Criteria): 大仓库长任务中断后可恢复或安全重建（`index --resume` 断点续传，不重复全量）。
- [x] **AC2** (PRD §Constraints 性能/资源): daemon idle 内存 < 300MB、基础索引 < 2GB、单次搜索额外 < 200MB（工程目标，真实大仓库基准）。
- [x] **AC3** (PRD §Implementation Phases Phase 8 Exit Criteria / §Success Metrics 反指标): secret redaction / export / audit log 回归测试通过（denylist/secret scan 不被性能优化绕过）。
- [x] **AC4** (PRD §User Flow 异常流): 索引中断进入长任务模式（进度显示/可中断/可恢复），大规模变更自动降级后台任务。

## 7. SDD / BDD / TDD Traceability

| Acceptance Criterion | BDD Scenario | TDD Test | Integration / E2E Test | Verification | Status |
|---|---|---|---|---|---|
| AC1 中断可恢复/续传 | SCEN-8.2.1 | TEST-8.2.1 | - | unit-test | Done |
| AC2 资源占用达标 | SCEN-8.2.2 | TEST-8.2.2 | - | unit-test | Done |
| AC3 secret/export 回归 | SCEN-8.2.3 | TEST-8.2.3 | - | unit-test | Done |
| AC4 长任务模式降级 | SCEN-8.2.4 | TEST-8.2.4 | - | unit-test | Done |
...
- **完成日期**：2026-05-23
- **改动文件**：
  - `docs/specs/tasks/task-8.2-reliability.md`（修改）
  - `test/features/reliability.feature`（修改）
  - `internal/reliability/reliability_test.go`（新增）
  - `internal/reliability/reliability.go`（新增）
  - `internal/cli/index_test.go`（新增）
  - `internal/cli/index.go`（新增）
  - `internal/cli/cli.go`（修改）
  - `internal/cli/cli_test.go`（修改）
- **commit 列表**：
  - `f7447c1` `docs(spec): task-8.2 §2A 自审 Ready`
  - `5c92912` `test(reliability): task-8.2 resume/resource RED`
  - `8b0be37` `feat(reliability): 实现 task-8.2 resume/resource guard`
- **§9 Verification 结果**：
  - install: ✅ `go mod download && cargo fetch`
  - typecheck: ✅ `go vet ./... && cargo check --workspace`
  - unit-test: ✅ `go test ./... && cargo test --workspace`（Go 全包通过；Rust 60 passed / 0 failed）
- **剩余风险 / 未做项**：真实 10 万 chunk release smoke 与跨平台资源观测留给 task-8.3 收口。
- **下游 task 影响**：task-8.3 需要复用 `internal/reliability` 的资源与安全 guard 作为 v0.1 release evidence。
