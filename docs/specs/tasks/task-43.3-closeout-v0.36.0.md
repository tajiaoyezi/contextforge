...
- [x] **AC1**（smoke v33[52/52] + no-regression）: `scripts/console_smoke.sh` v33[52/52] indexing replay splice 可达断言（不可达诚实归因 unit）+ `internal/cli/smoke_syntax_test.go` `TestTask433` markers 同步（no-regression [37/37]..[51/51] 不溯改） — verified by **TEST-43.3.1a**（`bash -n scripts/console_smoke.sh` + `TestTask433` 绿）+ **TEST-43.3.1b**（smoke v33[52/52] 可达断言 / 不可达 doc/status 诚实归因）
- [x] **AC2**（v0.36.0 release docs + ADR-048 ratify + ADR-038 Amendment + roadmap/adapter）: `docs/releases/v0.36.0-{evidence,artifacts}.md` + README/RELEASE_NOTES v0.36 段 + ADR-048 Proposed→Accepted（逐 D 据真实测试）+ ADR-038 add-only Amendment + roadmap §3.25/§4 add-only + s2v-adapter add-only + phase §6 闭合 — verified by **TEST-43.3.1c**（release docs / ADR ratify / Amendment / roadmap / adapter 全在场）
- [x] **AC3**（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by **TEST-43.3.2**（= LAST，CI spec-lint job 全绿）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-43.3.1a | smoke v33[52/52] `bash -n` 通过 + `TestTask433` 绿（[52/52] + markers indexing-replay/splice/list_since/TEST-43.1. + no-regression [37/37]..[51/51] 不溯改） | `scripts/console_smoke.sh` + `internal/cli/smoke_syntax_test.go` | Done |
| TEST-43.3.1b | smoke v33[52/52] indexing replay splice 可达断言（REAL 模式 since_ts>0 订阅者收到 evt-idx-* 事件序列）/ 不可达 doc/status 诚实归因 unit TEST-43.1.2（ADR-013 不伪造） | `scripts/console_smoke.sh` | Done（doc/status 归因 unit，REAL-mode e2e honest-defer） |
| TEST-43.3.1c | v0.36.0 release docs（evidence/artifacts/README/RELEASE_NOTES）+ ADR-048 Proposed→Accepted Ratification + ADR-038 add-only Amendment + roadmap §3.25/§4 + s2v-adapter + phase §6 闭合 全在场 | docs（多文件） | Done |
| TEST-43.3.2 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威）（= LAST） | `scripts/spec_drift_lint.sh` | Done（CI spec-lint job 全绿） |
...
**改动文件**：
- `scripts/console_smoke.sh`（v32→v33 + step [52/52] indexing-replay-splice doc/status + v33 changelog 注释段）
- `internal/cli/smoke_syntax_test.go`（新 `TestTask433_SmokeV33IndexingReplaySpliceStep` + no-regression [37/37]..[51/51]）
- `docs/releases/v0.36.0-evidence.md`（新增）+ `docs/releases/v0.36.0-artifacts.md`（新增）
- `README.md`（v0.36 段）+ `RELEASE_NOTES.md`（v0.36.0 段）
- `docs/decisions/adr-048-indexing-replay-splice.md`（Proposed→Accepted + §Ratification）
- `docs/decisions/adr-038-governance-debt-cleanup-2.md`（add-only `## Amendment (Phase 43 / v0.36.0)`）
- `docs/roadmap.md`（§3.25 推进记录 + §4 backlog add-only）+ `docs/s2v-adapter.md`（Phase 43 / Task / ADR-048 / BDD 状态翻新）
- `docs/specs/phases/phase-43-governance-debt-cleanup-4.md`（Status Done + §6 AC 勾选）
- `docs/specs/tasks/task-43.1-indexing-replay-splice.md`（Status Done + §10 已在 PR #276 回填）

**commit 列表**：
- `<本 PR>` feat/closeout: task-43.3 v0.36.0 closeout（smoke v33 + release docs + ADR ratify + Amendment + roadmap/adapter）

**§9 Verification 结果**：
- install: skipped（0 代码 dep 改动，纯 docs + smoke 脚本）
- lint: ✅ `bash -n scripts/console_smoke.sh` exit 0
- typecheck: N/A（无 Rust/Go 代码改动，仅 smoke_syntax_test 新增 test 已 `go test` 验证）
- unit-test: ✅ `go test ./internal/cli/ -run TestTask433` PASS
- integration / e2e / build: N/A（closeout docs + smoke）
- coverage: N/A
- runtime-smoke: skipped（smoke v33[52/52] 是 doc/status 归因 unit；REAL-mode subscribe-stream e2e 🟡 honest-defer `[SPEC-DEFER:phase-future.indexing-replay-daemon-e2e]`）
- manual: ✅ release docs / ADR ratify / Amendment / roadmap / adapter 全在场（TEST-43.3.1c）；ADR-048 据 task-43.1 unit 真实测试 ratify（D1-D3 🟢 / D4 live daemon e2e 🟡 honest-defer）

**剩余风险 / 未做项**：live daemon restart-then-replay e2e `[SPEC-DEFER:phase-future.indexing-replay-daemon-e2e]`（须 running daemon + 跨 restart 双窗口）；memory-actor-all-rpc（独立 phase）；memory actor 认证身份 `[SPEC-DEFER:phase-future.memory-actor-authenticated-identity]`。真实 v0.36.0 tag/release 待用户 outward-facing 授权（ADR-012）。

**下游 task 影响**：无（Phase 43 收口；v0.36.0 tag/release 待用户授权后 post-tag-push backfill 真实 tag SHA / run-id / digest / tlog）。
