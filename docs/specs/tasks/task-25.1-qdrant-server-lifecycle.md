...
- [x] **AC1**: feature `vector-qdrant` 下连接配置校验——合法配置（url 非空 / dim>0 / collection 名非空 / metric 受支持）`validate()` Ok；非法（url 空 / dim=0 / collection 名空）→ 可识别 Err（不连 server，纯函数）— verified by **TEST-25.1.1**
- [x] **AC2**: health-probe 无 server 时返回可识别 unreachable 状态——`health()` 连一个不存在端点返 unreachable（不 panic、不静默成功），可被调用方识别 — verified by **TEST-25.1.2**
- [x] **AC3**: collection ensure-create 决策三分支——`decide_ensure`：存在且 dim/metric 匹配 → Reuse、不存在 → Create、存在但 dim/metric 不匹配 → Error（喂入构造的 describe 响应，不连 server）— verified by **TEST-25.1.3**
- [x] **AC4**: 真实 KNN over live qdrant 诚实延后 + 不破坏三 trait 签名——`[SPEC-DEFER:phase-future.qdrant-server-lifecycle]` 在 spec/spike 如实标（CI 无 live server，禁伪造 live-server 通过 ADR-013）；生命周期方法为 inherent method 不改三 trait 签名 — verified by **TEST-25.1.4**
- [x] **AC5**: 既有不退化 + D2 lint — 默认 `cargo test --workspace`（无 vector feature）全 PASS + 0 新依赖；`cargo test --workspace --features vector-qdrant` 既有 qdrant 契约不退化；`go test ./...` 不受影响（本 PR 零 Go delta）；`bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by **TEST-25.1.5** + §10 实测

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-25.1.1 | feature `vector-qdrant` 连接配置校验（合法 Ok / url 空·dim=0·名空 Err，纯函数不连 server） | `core/src/retriever/vector/qdrant.rs`（`mod tests`） | Done |
| TEST-25.1.2 | health-probe 无 server 返可识别 unreachable 不 panic | `core/src/retriever/vector/qdrant.rs`（`mod tests`） | Done |
| TEST-25.1.3 | collection ensure-create 决策 reuse·create·error 三分支（构造 describe 响应） | `core/src/retriever/vector/qdrant.rs`（`mod tests`） | Done |
| TEST-25.1.4 | live-server 集成诚实延后标注 + 不破坏三 trait 签名（trait object 构造） | `core/src/retriever/vector/qdrant.rs`（`mod tests`）+ spec/spike | Done |
| TEST-25.1.5 | 默认 `cargo test --workspace` 0 failed + `--features vector-qdrant` 不退化 + D2 lint 0 未标注命中 | 全 Rust + `scripts/spec_drift_lint.sh` | Done |
...
- **完成日期**: 2026-05-31
- **改动文件**:
  - `core/src/retriever/vector/qdrant.rs`（修改）— 新增 `QdrantConnConfig`（url/timeout/api_key/tls + `from_env` + `validate`）+ `QdrantHealth` + `CollectionDesc` + `EnsureAction` + `decide_ensure` 纯函数 + `metric_to_distance` + `collection_desc_from_info` + `QdrantBackend::connect` + `health()`；`open()` 重写为 ensure-create（不再无脑 drop+create）；`import VectorMetric`；新增 4 个 feature-gated 单测
- **commit 列表**:
  - `ab4dae2` test(retriever): TEST-25.1.1~25.1.4 RED — qdrant 生命周期契约层
  - `b903906` feat(retriever): qdrant 生命周期契约层（connect/health/decide_ensure + ensure-create open），通过 TEST-25.1.1~25.1.4
  - （本 commit）docs(spec): 回填 task-25.1 §10 + Status → Done
- **§9 Verification 结果**（ADR-013 真实非合成，本机；feature `vector-qdrant` 本机可构建——qdrant-client 1.18 携预生成 protobuf，无需 protoc）:
  - unit-test: `cargo test -p contextforge-core --features vector-qdrant retriever::vector::qdrant` **4 passed / 0 failed**（TEST-25.1.1 config 校验 / 25.1.2 health 无 server Unreachable 不 panic / 25.1.3 decide_ensure reuse·create·error 三分支 / 25.1.4 三 trait object 构造不破坏签名）；RED→GREEN 已复核（RED 3 failed on todo!）
  - unit-test: `cargo test --workspace`（默认无 vector feature）0 failed（qdrant.rs 全 `#[cfg(feature="vector-qdrant")]` gated，默认构建不编译，逐字节不变）
  - integration: `go test ./...` 0 FAIL（本 PR 零 Go delta）
  - build: ✅ 默认构建 0 新依赖（`core/Cargo.toml`/`Cargo.lock` 未改，qdrant-client 1.18 自 task-18.4 起即 optional）
  - lint: ✅ `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及 docs/specs 行 0 未标注命中（本机 scoped 复核 + CI spec-lint gate）
- **剩余风险 / 未做项**: 真实 live qdrant server 端到端 connect→ensure-create→upsert→KNN 集成 **诚实延后** `[SPEC-DEFER:phase-future.qdrant-server-lifecycle]`（CI 无 live server，禁伪造 live-server 通过 ADR-013）；契约层（config 校验 / health unreachable / decide_ensure 纯函数）在不连 server 下 deterministic 单测覆盖。`open()` Reuse 分支的 id_map 自 server 重建属 live 集成延后项；named-vector collection 描述提取返回 None（契约层不解析）。集群/复制/部署拓扑 `[SPEC-DEFER:phase-future.qdrant-deployment-topology]`。
- **下游 task 影响**: task-25.3（closeout 据本生命周期层评估生产 backend 选择矩阵 qdrant 档 caveat + smoke v15）。可与 task-25.2（lancedb）并行（写路径不相交）。
