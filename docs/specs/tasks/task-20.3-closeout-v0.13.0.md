...
- [ ] **AC1**: `scripts/console_smoke.sh` v10 通过 `bash -n`（exit 0）；step 29 升级为 console-api `/v1/search?semantic=true` 真实语义断言（`retrieval_method` 语义标记 + result item `vector_score` provenance）；既有 step 1-28 + step 30 不退化 — verified by **TEST-20.3.1**
- [ ] **AC2**: v0.13.0 release docs 齐备（`docs/releases/v0.13.0-{evidence,artifacts}.md` + `README.md` v0.13 段 + `RELEASE_NOTES.md` v0.13.0 段）；evidence 含 task 表 / CI / AC 达成 / 平台矩阵 / upgrade-rollback / §tag-backfill 待回填段 — verified by **TEST-20.3.2**
- [ ] **AC3**: ADR-024 据 task-20.1 实测 Status `Proposed → Accepted`（或记录维持）；phase-20 §6 AC1-5 全 `[x]` + Status `Draft → Done`；adapter Phase 20 `Draft → Done` + Tasks `0 → 3` + ADR-024 索引 + v0.12.0 caveat 解除注 — verified by **TEST-20.3.3**
- [ ] **AC4**: 既有不退化 — `go test ./...` + `cargo test --workspace` 全 PASS — verified by **TEST-20.3.4** + §10
- [ ] **AC5**: ADR-014 D1-D5 第十一次激活全通过（D1 phase§6↔task§6 mapping 表 + D2 lint `--touched origin/master` 0 未标注命中 + D3 verified-by + D4 自治 + D5 历史 Phase 1-19 不溯改）— verified by **TEST-20.3.5** + 本 closeout PR body

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-20.3.1 | smoke v10 `bash -n` + step 29 真实语义断言 + 既有 step 不退化 | `internal/cli/smoke_syntax_test.go` + `scripts/console_smoke.sh` | Planned |
| TEST-20.3.2 | v0.13.0 release docs 齐备 + 结构校验 | `docs/releases/v0.13.0-*.md` + README + RELEASE_NOTES | Planned |
| TEST-20.3.3 | ADR-024 ratify + phase-20 闭合 + adapter 更新 | `docs/decisions/adr-024-*.md` + phase-20 spec + s2v-adapter | Planned |
| TEST-20.3.4 | `go test ./...` + `cargo test --workspace` 0 failed | 全 Go + Rust | Planned |
| TEST-20.3.5 | ADR-014 D1-D5 record（mapping + D2 lint） | 本 closeout PR body | Planned |
...
- **Status**: 待实施（Draft）。实施完成后按 6 项回填，含 smoke v10 实跑结论（合规环境 `CONSOLE_REAL_SMOKE_EXIT=0` / WSL 受阻如实记录）+ ADR-024 ratify 结论 + v0.13.0 tag/backfill 状态（用户授权后）。
