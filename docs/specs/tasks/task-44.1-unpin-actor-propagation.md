...
(6) **B5 Go 透传链**：types.go `Unpin(id)` → `Unpin(id, actor)` + grpcclient.go `pb.UnpinMemoryRequest{MemoryId, Actor}` + handlers.go `handleMemoryUnpin` 读 `X-Actor`（:559）+ memstore.go `Unpin(id, actor)` + degraded fallback（`console_api_serve_degraded.go`）同步。
...
- Go 4 处透传（types/grpcclient/handlers X-Actor/memstore）+ degraded fallback 同步
...
- [x] **AC1**（unpin actor 进 audit/event source 🟢）: unpin(actor) → audit source = actor / event source = actor — verified by **TEST-44.1.1**
- [x] **AC2**（pin 顺带闭环 🟢）: pin(actor) → audit source = actor（消除 pin audit/event 不归因残余不对称） — verified by **TEST-44.1.2**
- [x] **AC3**（空 actor byte-equiv 🟢）: 空 actor → audit source "console-api" / event source "contextforge-core"（各自 byte-identical 现状） — verified by **TEST-44.1.3**
- [x] **AC4**（Go handleMemoryUnpin 读 X-Actor 🟢）: handler 读 X-Actor 透传 Unpin(id, actor) — verified by **TEST-44.1.4**
- [x] **AC5**（ADR-014 D2 lint）: PR 触及行 0 未标注命中 — verified by **TEST-44.1.5**（= LAST，CI spec-lint）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-44.1.1 | unpin(actor="bob") → audit source "bob" / event source "bob"（audit/event source 归因真实调用方） | `core/src/data_plane/memory.rs`（同源 test） | Done |
| TEST-44.1.2 | pin(actor="alice") → audit source "alice" / event source "alice"（pin 顺带闭环，消除 audit/event 不归因残余不对称） | `core/src/data_plane/memory.rs`（同源 test） | Done |
| TEST-44.1.3 | 空 actor unpin/pin → audit source "console-api" / event source "contextforge-core"（各自 byte-equiv 现状） | `core/src/data_plane/memory.rs`（同源 test） | Done |
| TEST-44.1.4 | Go handleMemoryUnpin 读 X-Actor header 透传 Unpin(id, actor) + grpcclient 填 pb.UnpinMemoryRequest.Actor | `internal/consoleapi/unpin_actor_test.go`（Go 源码 grep） | Done |
| TEST-44.1.5 | D2 lint `--touched origin/master` 0 未标注命中（= LAST） | `scripts/spec_drift_lint.sh` | Done（CI spec-lint） |
...
**改动文件**：
- `proto/contextforge/console_data_plane/v1/console_data_plane.proto`（UnpinMemoryRequest add-only actor=2）+ `console_data_plane.pb.go` + `console_data_plane_grpc.pb.go`（buf generate）
- `core/src/data_plane/memory.rs`（emit_audit_and_event 加 actor 参数 + audit source 用 actor + build_memory_event 加 actor 参数 + event source 用 actor + unpin handler 透传 + pin handler 顺带传 req.actor + deprecate/softdelete/harddelete 传 "" byte-equiv + 既有 unpin 测试补 ..Default::default() + TEST-44.1.1/.2/.3）
- `internal/consoleapi/types.go`（MemoryClient.Unpin 加 actor）+ `grpcclient/grpcclient.go`（memoryClient.Unpin 加 actor + pb.Actor）+ `handlers.go`（handleMemoryUnpin 读 X-Actor）+ `memstore.go`（MemMemoryStore.Unpin 加 _actor）+ `memstore_test.go`（既有 Unpin 调用补 ""）+ `unpin_actor_test.go`（TEST-44.1.4 源码 grep）+ `cli/console_api_serve_degraded.go`（degraded Unpin 加 _actor）

**commit 列表**：
- `<本 PR>` feat(memory): task-44.1 unpin-actor-propagation

**§9 Verification 结果**：
- install: skipped（0 新 dep）
- lint: ✅ clippy 0 warning；gofmt（CI Linux LF 干净，Windows CRLF 假阳性）
- typecheck: ✅ cargo check + go build + go vet pass
- unit-test: ✅ lib 229→232（+3：test_44_1_1/.2/.3）+ go test（含 TestTask441_UnpinActorPropagationWired）全绿
- integration / e2e / build: ✅ cargo test --workspace 全绿 + go test ./... 全绿
- coverage: N/A
- runtime-smoke: skipped（smoke v34[53/53] 在 task-44.3 closeout）
- manual: ✅ unpin actor 进 event source（TEST-44.1.1 "bob"）+ pin 顺带闭环（TEST-44.1.2 "alice"）+ 空 actor byte-equiv（TEST-44.1.3 "contextforge-core"）+ Go 透传链源码 grep（TEST-44.1.4）

**剩余风险 / 未做项**：认证身份 `[SPEC-DEFER:phase-future.memory-actor-authenticated-identity]`（须 console-api 鉴权层）；deprecate/softdelete/harddelete 真实 actor 透传 `[SPEC-DEFER:phase-future.memory-actor-all-rpc]`（本 task 仅做 emit_audit_and_event actor 参数共用基础，这 3 RPC 传 "" byte-equiv）。

**下游 task 影响**：task-44.3 closeout（ADR-049 ratify + ADR-032/045 Amendment + smoke v34[53/53]）。
