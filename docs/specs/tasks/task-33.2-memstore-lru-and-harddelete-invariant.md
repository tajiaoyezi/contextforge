...
- **B3 handleMemoryPin strict-400 — 经契约审计为据实不改（lenient 是 deliberate contract）**：`internal/consoleapi/handlers.go` `handleMemoryPin`（`:525-549`）对 malformed / empty / absent body 故意回落 `pin=true`（`:536` 初值 + `:540-542` 仅当 decode 成功且 `body.Pin != nil` 才覆盖）以保留 v0.7 宽松契约（doc-comment `:519-524` 明记）；此为 task-17.1 / ADR-022 D2 的既定决策。改为 400 会违反 ADR-004（默认行为不变）且推翻已 Accepted 的 ADR-022 D2。本 task 记为**诚实 non-change**（无代码改动），仅在 ADR-038 D4 记其 lenient by design。
