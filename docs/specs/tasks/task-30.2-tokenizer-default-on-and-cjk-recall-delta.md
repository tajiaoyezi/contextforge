...
- [x] AC1（🟡 真实 recall delta，扩 CJK golden）: 扩充后 golden（16 q / 14 file）经 phase24-style harness 实测 **default / bigram / 真分词**真实 recall——default recall@5/@10=0.8750 mrr=0.8750；bigram(code_cjk) 1.0/1.0 mrr=0.9375；segmenter(cjk_seg) 1.0/1.0 mrr=0.9375。**delta(seg−bigram)=+0.0000（全指标）**——小语料 file-level 召回真分词与 bigram 持平（两者均完整召回 CJK case）；delta(seg−default)=+0.1250 recall / +0.0625 mrr。**诚实结论（ADR-013，不外推）**：小语料下真分词相对 bigram **无 file-level 召回提升**（真分词价值在 token 洁净/精度而非此规模召回；bigram 0-dep fallback 仍有价值）——如实记录零 delta，不伪造提升 — verified by TEST-30.2.1（harness 实测）
- [x] AC2（🟢 reindex/migration + schema-driven 对称）: `IndexSession::reindex_with_tokenizer(data_dir, coll, new_tokenizer)` 真实迁移工具——读 SQLite chunk 真相源 + drop/重建 Tantivy 绑定 new analyzer + 重加 chunk（SQLite 不动，向后兼容）；config 采**方案 B schema-driven 对称**——`RetrieverConfig.tokenizer` doc 注明 vestigial（search 据 schema/`meta.json` 绑定派生，非 config），切 analyzer 经 reindex 工具非改 config；index/query 双站点对称保持（task-24.1 R4 / task-30.1） — verified by TEST-30.2.2（default-build reindex round-trip PASS）+ TEST-30.2.2b（feature reindex PASS）
- [x] AC3（🟢 扩 golden 经 Go 守门）: 扩充后 `test/fixtures/eval/golden-semantic.jsonl`（11→16 行，+5 CJK case add-only）经 Go `ValidateGoldenSemantic`（schema / dup / category，`internal/eval/eval.go:231-280`）通过；既有 11 行不删改；`go test ./internal/eval/...` 绿 — verified by TEST-30.2.3（PASS）
- [x] AC4（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by TEST-30.2.4（PASS）

> **tokenizer-default-on 评估结论（ADR-013，不强推）**：默认翻转（default flip）须 reindex 既有索引 + 用户决策面，本 task 提供**真实迁移工具**（`reindex_with_tokenizer`）+ schema-driven 对称文档化，但**默认仍保 opt-in**（不在本 task 自行翻默认）——full default-on 翻转据「迁移工具已备 + 翻默认是产品决策」诚实延后 `[SPEC-DEFER:phase-future.tokenizer-default-on]`。

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-30.2.1 | 扩 CJK golden 经 phase24-style harness 跑 default/bigram/真分词真实 recall delta（seg−bigram=+0.0000 / seg−default=+0.1250，小语料不外推，ADR-013） | `core/examples/phase24_tokenizer_recall.rs`（+真分词 path）+ `test/fixtures/eval/golden-semantic.jsonl` + §10 | Done (实测) |
| TEST-30.2.2 | `reindex_with_tokenizer` 工具按目标绑定重建（default→code_cjk）+ 既有默认索引向后兼容 + index/query 对称 round-trip（经 Retriever 只读查询） | `core/src/indexer/mod.rs`（默认构建 test） | Done (PASS) |
| TEST-30.2.2b | reindex 到真分词绑定（default→cjk_segmenter，feature）真词命中 | `core/src/indexer/mod.rs`（feature-gated test） | Done (PASS) |
| TEST-30.2.3 | 扩充后 golden CJK case 经 Go `ValidateGoldenSemantic`（schema/dup/category，eval.go:231-280）通过 + 既有 11 行 add-only 不改 | `internal/eval/eval.go` + `test/fixtures/eval/golden-semantic.jsonl` | Done (PASS) |
| TEST-30.2.4 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威） | `scripts/spec_drift_lint.sh` | Done (PASS) |
...
**实际改动文件**：
- `test/fixtures/eval/golden-semantic.jsonl`——add-only +5 CJK case（可观测性/增量索引/生命周期/跨进程/向量检索，各映射强关联 ADR 文件）；既有 11 行不删改（11→16）。
- `core/src/indexer/mod.rs`——`IndexSession::reindex_with_tokenizer(data_dir, coll, new_tokenizer)` 真实迁移工具（读 SQLite chunk + drop/重建 Tantivy 绑定 new analyzer + 重加 chunk，SQLite 不动）+ TEST-30.2.2（default→code_cjk round-trip）+ TEST-30.2.2b（feature default→cjk_segmenter）。
- `core/src/retriever/mod.rs`——`RetrieverConfig.tokenizer` doc 注明 vestigial / schema-driven 对称（方案 B：search 据 schema/`meta.json` 绑定派生 analyzer，切 analyzer 经 reindex 工具非改 config）。
- `core/examples/phase24_tokenizer_recall.rs`——`#[cfg(feature = "cjk-segmenter")]` 第三 path（cjk_segmenter）+ default/bigram/真分词三方 recall delta 输出 + per-cjk-case rank 对比。
- `docs/specs/tasks/task-30.2-...md`——§6/§7/§10 回填实测。

**§9 Verification 实测证据**：
- AC1（🟡 真实 recall delta，16 q / 14 file，`cargo run --features cjk-segmenter --example phase24_tokenizer_recall`）：**default 0.8750/0.8750 mrr 0.8750 → bigram 1.0/1.0 mrr 0.9375 → segmenter 1.0/1.0 mrr 0.9375**。delta(seg−bigram)=**+0.0000 全指标**（小语料 file-level 召回真分词与 bigram 持平，两者均完整召回扩充后 CJK case）；delta(seg−default)=+0.1250 recall / +0.0625 mrr。诚实结论：小语料下真分词相对 bigram **无 file-level 召回提升**（真分词价值在 token 洁净/精度，非此规模召回；bigram 0-dep fallback 仍有价值），如实记录零 delta，不外推、不伪造（ADR-013）。
- AC2（🟢）：`cargo test -p contextforge-core --lib test_30_2_2` 默认构建 **1 passed**（reindex default→code_cjk：迁移前 default 绑定 `置加` 不命中 → 迁移后 bigram `置加`+code `config` 命中，chunk 保留）；`--features cjk-segmenter` **2 passed**（+30.2.2b reindex→cjk_segmenter 真词 `配置` 命中）。
- AC3（🟢）：`go test ./internal/eval/...` PASS（扩充后 16 行经 `ValidateGoldenSemantic` schema/dup/category 守门，既有 11 行 add-only 不改）。
- 不退化：`cargo test --workspace`（默认无 feature）0 failed；`cargo clippy --workspace --all-targets -- -D warnings`（默认）+ `cargo clippy --features cjk-segmenter --lib --tests --examples -- -D warnings` 均 0 warning；0 新 dep（jieba 自 task-30.1 即 optional）。
- AC4（🟢）：spec-lint `--touched origin/master` 0 未标注命中。
- **tokenizer-default-on 评估结论**：提供真实 `reindex_with_tokenizer` 迁移工具 + schema-driven 对称文档化（方案 B）；**默认仍保 opt-in 不自行翻默认**——full default flip 据「迁移工具已备 + 翻默认是产品决策」诚实延后 `[SPEC-DEFER:phase-future.tokenizer-default-on]`（ADR-013 不强推不达项）。
