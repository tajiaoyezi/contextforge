...
- [x] AC1（真分词 token stream，🟢 under `--features cjk-segmenter`）: 新 `cjk_segmenter` analyzer（jieba-rs 0.7.4）对 `配置加载` 切出**真词边界** `配置`/`加载`（token 集合区别于 bigram 的 `配置`/`置加`/`加载`，无 `置加` 滑窗噪声）；混合 `getConfig配置加载` → code 段 get/config + CJK 真词 配置/加载 — verified by TEST-30.1.1（PASS）
- [x] AC2（index/query 双站点注册对称，🟢）: 新 analyzer 名在 index 站点 `open_with_tokenizer:442` 与 query 站点 `open_with_config:250` **同时注册**（feature-gated）；opt-in collection（IndexSession 写入 → Retriever 查询）round-trip 真词 `配置` 双站点均命中（无静默召回退化，task-24.1 R4） — verified by TEST-30.1.2（PASS）
- [x] AC3（默认构建不变 + 0 新 dep，🟢）: 默认构建（无 `cjk-segmenter` feature）默认分词 + 6 字段 schema 不变；`cargo test --workspace`（无 feature）0 failed、不编译 jieba-rs（0 新 dep at default features）；`code_cjk` bigram fallback 保留 — verified by TEST-30.1.3（默认 test PASS）
- [x] AC4（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by TEST-30.1.4（PASS）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-30.1.1 | `--features cjk-segmenter` 下真分词 token stream 单测：`配置加载 → 配置/加载`（真词边界，区别于 bigram `配置/置加/加载`） | `core/src/indexer/mod.rs`（`#[cfg(feature = "cjk-segmenter")]` test） | Done (PASS) |
| TEST-30.1.2 | index/query 双站点注册对称单测：新 analyzer 名在 `:442` + `:250` 同时注册；opt-in collection round-trip（IndexSession 写 + Retriever 查）解析 + 命中无静默退化 | `core/src/indexer/mod.rs` + `core/src/retriever/mod.rs`（feature-gated test） | Done (PASS) |
| TEST-30.1.3 | 默认构建（无 feature）无回归：默认分词 + 6 字段 schema + 0 新 dep；`cargo test --workspace` 不受影响；`code_cjk` fallback 保留 | `core/src/indexer/mod.rs`（默认 test） | Done (PASS) |
| TEST-30.1.4 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威） | `scripts/spec_drift_lint.sh` | Done (PASS) |
...
- **实际改动文件**:
  - `core/Cargo.toml`——**main-agent R7 chore（ADR-008 add-only）**：新增 optional `jieba-rs = { version = "0.7", optional = true }`（pure-Rust，内嵌词典，无 C/build 前置；选 jieba 而非 lindera 因更轻）+ `cjk-segmenter = ["dep:jieba-rs"]` feature 行。默认 `default = []` 不含 → 默认构建不编译 jieba（0 新 dep at default features，ADR-004）。`Cargo.lock` 新增 jieba-rs 0.7.4 + 其传递依赖（optional，pin via R7；默认构建不拉取编译）。
  - `core/src/indexer/mod.rs`——新增 `CJK_SEGMENTER_TOKENIZER = "cjk_segmenter"` 常量（恒在，与 `:183` 并列）+ `build_tantivy_schema` opt-in 分支扩为 `code_cjk || cjk_segmenter`（按名绑定 content 字段）+ `#[cfg(feature = "cjk-segmenter")]` `tokenize_cjk_segmenter`（CJK run 经 `jieba.cut(run, false)` 真分词，code 段复用 `emit_code_segment`，offset 真实）+ `CjkSegmenterTokenizer`（复用 `CodeCjkTokenStream`）+ `build_cjk_segmenter_analyzer`（镜像 `:364-369` filter 链）+ `register_cjk_segmenter`（镜像 `:373-377`）+ index 站点 `open_with_tokenizer` 注册调用（feature-gated）+ TEST-30.1.1/30.1.2。
  - `core/src/retriever/mod.rs`——query 站点 `open_with_config` 对称注册 `register_cjk_segmenter`（feature-gated）。
- **§9 Verification 实测证据**:
  - AC1：`cargo test -p contextforge-core --features cjk-segmenter --lib test_30_1` → **2 passed; 0 failed**。jieba 0.7.4 实测 `配置加载 → [配置, 加载]`（真词边界，无 `置加`），与 bigram `[配置, 置加, 加载]` 显式 `assert_ne!` 区分；混合 `getConfig配置加载` → get/config + 配置/加载。
  - AC2：TEST-30.1.2 双站点 round-trip——IndexSession（index 站点）写 `配置加载模块` + Retriever（query 站点 `open_with_config`）查 `配置` 双站点均命中（无静默退化）。
  - AC3：`cargo test --workspace`（无 feature）0 failed；默认构建不编译 jieba（0 新 dep at default features）；`code_cjk` bigram fallback 保留（TEST-24.1.2 仍绿）。
  - AC4：`cargo clippy --workspace --all-targets -- -D warnings`（默认）+ `cargo clippy --features cjk-segmenter --lib --tests -- -D warnings` 均 0 warning；spec-lint `--touched origin/master` 0 未标注命中。
  - 分词库构建：jieba-rs 0.7.4 在 `x86_64-pc-windows-msvc`（rustc 1.95.0）`cargo build --features cjk-segmenter` exit 0——**纯 Rust 无构建受阻**（未触发 R2 🔴 lindera 重词典风险，选 jieba 兑现）。
- **诚实口径**：本 task 落 analyzer seam + 双站点对称 + deterministic 分词单测；**真实 recall delta 由 task-30.2 承担**（不在本 task 产出召回数值，ADR-013）。`Cargo.lock` 含 jieba 条目是 optional dep 正常记录（同 ureq 先例），默认构建不编译。
