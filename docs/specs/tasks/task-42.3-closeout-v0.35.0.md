# Task `42.3`: `closeout-v0.35.0 — smoke v31→v32[51/51]（REAL 模式 source_type 真实过滤端到端：复用既有 index-job-real 全 markdown fixture（runner/JobRunner 文档于 .md → source_type=doc），POST /v1/search?source_type=doc 保留 doc hit、?source_type=code 过滤掉它、空 filter 返全部）+ TestTask423 no-regression（[37/37]..[50/50] 不溯改）+ v0.35.0 release docs + ADR-047 据真实测试 ratify + ADR-037 add-only Amendment（source_type no-op supersede / agent_scope no-op 保持）+ roadmap §3.24/§4 add-only + s2v-adapter add-only`
...
- **B1 smoke 真实过滤断言（非保形）**：current smoke v31[50/50]（Phase 41）；本 task v32[51/51] REAL 模式复用既有 `index-job-real` **全 markdown** fixture（`runner`/JobRunner 文档于 .md → `classify_source_type` 得 source_type=doc），`POST /v1/search?source_type=doc` 保留 doc chunk（`source_file_type="doc"`）、`?source_type=code` 过滤掉它（fixture 无 code 文件）、空 filter 返全部——distinguishing 断言（证真实过滤生效，非 no-op：no-op 会两者皆返回该 chunk）。
...
(1) **smoke v32[51/51]**：`scripts/console_smoke.sh` banner v31→v32 + v32 changelog block + 新 step [51/51]（REAL 模式 source_type 真实过滤端到端：复用既有 `index-job-real` 全 markdown fixture / `?source_type=doc` 保留 doc hit / `?source_type=code` 过滤掉它 / 空 filter 返全部；不可达则 doc/status）；`internal/cli/smoke_syntax_test.go` 新 `TestTask423`（镜像 `TestTask413`）断言 [51/51] + markers + no-regression（denominators [37/37]..[50/50] 不溯改）。
...
- **B1 smoke distinguishing 断言**：REAL 模式（`MODE=real` && daemon succeeded）复用既有 `index-job-real` 全 markdown fixture（`runner`/JobRunner 文档于 .md → source_type=doc），`POST /v1/search?source_type=doc` 断言 `grep` 保留 doc chunk（`source_file_type="doc"`）、`?source_type=code` 断言过滤掉它（fixture 无 code 文件 → 空）；空 filter 返全部——distinguishing（source_type 过滤真实生效，非 no-op、非保形：no-op 会两者皆返回）；不可达 / non-real 模式 → echo doc/status（诚实归因到单测）。
...
- [x] **AC1**（smoke v32[51/51] + TestTask423 🟢）: `scripts/console_smoke.sh` banner v31→v32 + 新 step [51/51]（REAL source_type 真实过滤端到端：复用 `index-job-real` 全 markdown fixture / `?source_type=doc` 保留 doc hit / `?source_type=code` 过滤掉它 / 空 filter 全部，distinguishing）+ `internal/cli/smoke_syntax_test.go` `TestTask423`（[51/51] + markers + no-regression [37/37]..[50/50] 不溯改 + `bash -n`） — verified by **TEST-42.3.1**
- [x] **AC2**（v0.35.0 release docs + ADR-047 ratify + ADR-037 Amendment + roadmap/adapter）: `docs/releases/v0.35.0-{evidence,artifacts}.md` + README/RELEASE_NOTES v0.35 段（backfill marker）+ ADR-047 Proposed→Accepted（逐 D 据真实测试）+ ADR-037 add-only Phase-42 Amendment（source_type no-op supersede / agent_scope no-op 保持，不溯改正文）+ roadmap §3.24/§4 add-only + s2v-adapter Phase 42/Task/ADR-047/BDD 行 + phase §6 AC 勾选 + Status Done — verified by **TEST-42.3.1**（docs 一致性）+ closeout PR review
- [x] **AC3**（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中 — verified by **TEST-42.3.2**（= LAST）

## 7. 追踪表

| TEST-ID | 描述 | 落地文件 | Status |
|---|---|---|---|
| TEST-42.3.1 | smoke v32[51/51]（REAL `index-job-real` 全 markdown fixture → `?source_type=doc` 保留 JobRunner doc hit / `?source_type=code` 过滤掉它 distinguishing）+ `TestTask423`（[51/51] + markers chunk-source-type-filter/source_type/classify_source_type/TEST-42.1./TEST-42.2. + no-regression [37/37]..[50/50] 不溯改 + `bash -n`）+ release docs / ADR-047 ratify / ADR-037 Amendment / roadmap/adapter add-only 一致 | `scripts/console_smoke.sh` + `internal/cli/smoke_syntax_test.go` + release/ADR/roadmap/adapter docs | Done |
| TEST-42.3.2 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威）（= LAST） | `scripts/spec_drift_lint.sh` | Done |

## 8. Risks

- **R1（中）smoke 保形而非真实过滤断言**：若 step 仅断言响应 200 而不验证 source_type 过滤真实生效则空洞。
  - **缓解**：REAL 模式 distinguishing 断言（复用 `index-job-real` 全 markdown fixture，`?source_type=doc` grep 保留 doc hit / `?source_type=code` 过滤掉它），source_type 过滤真实生效才 PASS；不可达 / non-real → echo doc/status 诚实归因单测。stop-condition：step 保形 / 非 distinguishing 则 AC1 不标 `[x]`。
...
**§9 Verification（feat/task-42.3-closeout-v0.35.0，真实证据）**：
- AC1：`scripts/console_smoke.sh` v32 step `[51/51]`（REAL：复用 `index-job-real` 全 markdown fixture，`runner`/JobRunner 文档于 .md → source_type=doc → `?source_type=doc` 保留 JobRunner doc hit + `source_file_type=doc` / `?source_type=code` 过滤掉它——distinguishing：source_type no-op 会两者皆返回该 chunk；`bash -n` exit 0）+ `internal/cli/smoke_syntax_test.go` `TestTask423_SmokeV32ChunkSourceTypeFilterStep` PASS（`v32 (task-42.3)` header + markers `[51/51]`/chunk-source-type-filter/source_type/classify_source_type/TEST-42.1./TEST-42.2. + no-regression [37/37]..[50/50] + tokenizer-default-on）。**pre-tag 多 agent 对抗审查校正**：原 smoke 写「`?source_type=code` 保留 / `?source_type=doc` 过滤」与全 markdown fixture 相反（JobRunner 文档于 .md = doc 非 code），据实改为「doc 保留 / code 过滤」(fix PR)。
- AC2：`docs/releases/v0.35.0-{evidence,artifacts}.md`（tag/run/digest/tlog `<backfill: ...>` marker）+ `README.md` v0.35 段 + `RELEASE_NOTES.md` v0.35.0 段 + ADR-047 Proposed→Accepted（per-D Ratification）+ ADR-037 add-only `## Amendment (Phase 42 / v0.35.0)`（source_type no-op superseded / agent_scope no-op reaffirmed，不溯改 D-body）+ `docs/roadmap.md` §3.24 推进记录 + §4 add-only + `docs/s2v-adapter.md` Phase 42 Draft→Done/Tasks 0→3 + ADR-047 Proposed→Accepted + phase-42 §6 AC 全 `[x]` + Status Done。
- AC3：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- 全 phase 真实验证：`cargo test --workspace`（core lib 225）+ `go test ./...`（含 TestTask423 + TestTask422*）+ `cargo clippy --workspace --all-targets -- -D warnings` 0 warning + `go vet ./...` clean + gofmt clean（CR-strip 后）+ `bash -n scripts/console_smoke.sh` exit 0。默认构建 0 新 dep + 0 network + 0 schema migration + 空 filter byte-equiv（ADR-004/008/015）。

**实际改动文件**：`scripts/console_smoke.sh`（v32 [51/51]）+ `internal/cli/smoke_syntax_test.go`（TestTask423）+ `docs/releases/v0.35.0-{evidence,artifacts}.md`（new）+ `README.md` + `RELEASE_NOTES.md` + `docs/decisions/adr-047-...md`（ratify）+ `docs/decisions/adr-037-...md`（add-only Amendment）+ `docs/specs/phases/phase-42-...md`（§6 闭合）+ `docs/s2v-adapter.md` + `docs/roadmap.md`（§3.24/§4）+ task-42.1/42.2/42.3 spec（Status Done + §10）。

**真实 v0.35.0 tag/run/digest/tlog**：经用户授权 push（ADR-012）后 post-tag-push backfill 回填（release docs `<backfill: ...>` marker，ADR-013 不预填）。
