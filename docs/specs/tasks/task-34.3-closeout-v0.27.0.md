...
- [x] **AC3**（ADR-014 D2 lint）: `bash scripts/spec_drift_lint.sh --touched origin/master` PR 触及行 0 未标注命中（CI spec-lint 权威）— verified by **TEST-34.3.3**（= LAST）
...
- `cargo test -p contextforge-core --lib data_plane::search::tests::test_34_3_1` PASS——`get_source_chunk` workspace-isolation verify-only 守护测试三态（`workspace_id` 设 → 仅该 workspace chunk / 跨 workspace `chunk_id` → `Status::not_found` / 空 → 聚合全 workspace 探测命中），断言 task-12.2 `search.rs:421-423` 已在位隔离，0 生产代码改动（survey overstatement grounding 校正，ADR-013）。
...
- `cargo test -p contextforge-core --lib` 209 passed（含 `test_34_3_1`）+ `go test ./...` 全 PASS（含 TestTask343）；`cargo clippy --workspace --all-targets -- -D warnings` 0 warning。
- `bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- ADR-039 据 34.1-34.3 真实 CI ratify Proposed→Accepted（见 ADR-039 §Ratification）：D1 默认 BruteForce dim-agnostic no-op honest-caveat（默认协商接受任何 dim、无强制、byte-equiv）+ feature-backend live dim enforce 🟡 `[SPEC-DEFER:phase-future.vector-dim-feature-enforce]`、D2 env-wins + 无 `[vector]` 段 byte-equiv + Rust 0-dep 保持（无 toml dep）、D3 `get_source_chunk` isolation already-present verify-only（grounding 校正 survey overstated as gap）、D4 默认行为 + 0-dep + 0-network + 既有契约不变（ADR-004/008）如实记录（feature-enforce 🟡 honest-defer 不强 ratify，ADR-013）。
- ADR-037 add-only `## Amendment (Phase 34 / v0.27.0)`（vector-dim-auto-negotiation factory negotiate seam + `expected_dim` 默认 trait 方法 / vector-backend-config-file Go `[vector]`→env 跨进程桥 env-wins）——已落地（PR #224 `fed7a90` / #225 `a4ae446`），不溯改 D1-D5 正文 + 既有 Phase 32 Amendment 正文（ADR-014 D5）。
- roadmap §3.16 Phase 34 推进记录 + §4 add-only 新 backlog（vector-dim-feature-enforce / daemon-options-datadir / vector-config-file-rust-native）——已 add-only 落地。
