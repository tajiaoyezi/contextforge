...
|---|---|---|---|
| TEST-33.1.1 | L2 rowid-FIFO bound：`test_33_1_1_l2_cap_evicts_oldest_fifo`——`sqlite_put` 写后 `DELETE ... WHERE provider=? AND dim=? AND rowid NOT IN (SELECT rowid ... ORDER BY rowid DESC LIMIT cap)`，超 cap 时最旧 rowid 行被逐（重读 = miss）、仍在 cap 内文本命中；`cap==0` ⇒ 不限；隐式 rowid FIFO，0 新 dep + 0 schema migration | `core/src/embedding/cache.rs`（同源 test） | Done |
| TEST-33.1.2 | 默认行为不变：`test_33_1_2_default_l2_cap_keeps_modest_workload`——`DEFAULT_L2_EMBEDDING_CACHE_CAP=50_000` 下 modest workload 全保留（cache 7/7）；既有 L2 round-trip（TEST-22.2.3）+ L1 cap（TEST-31.2.1）+ TEST-22.2.* 不退化；公共构造源码兼容（add-only `with_sqlite_capacity` ctor，`with_sqlite` 不动）；`embedding_cache` schema 不变 + 0 新 dep | `core/src/embedding/cache.rs` | Done |
| TEST-33.1.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威，PR #218 spec-lint PASS）（= LAST） | `scripts/spec_drift_lint.sh` | Done |
...
**§9 Verification（v0.26.0 impl / PR #218，squash commit `3ce290a`，真实证据）**：
- AC1：`cargo test -p contextforge-core`（embedding cache）PASS——`test_33_1_1_l2_cap_evicts_oldest_fifo`：`sqlite_put` 写后 `DELETE ... WHERE provider=? AND dim=? AND rowid NOT IN (SELECT rowid ... ORDER BY rowid DESC LIMIT cap)`，超 cap 时最旧 rowid 行被淘汰（重读 = miss）、仍在 cap 内文本命中；`cap==0` ⇒ 不限。**实现校正（ADR-013 据实记，非规划稿口径）**：规划 §2/§5.2 设想「`COUNT(*)` 后 `DELETE ... ORDER BY rowid ASC LIMIT overflow`（逐 overflow 个最旧）」，实现采等价的「`rowid NOT IN (... ORDER BY rowid DESC LIMIT cap)`（保留 cap 个最新）」且按 `(provider, dim)` scope；语义一致（最旧 rowid = 最早插入先逐），隐式 rowid FIFO、0 新 dep + 0 schema migration。
- AC2：`cargo test -p contextforge-core`（embedding cache）PASS——`test_33_1_2_default_l2_cap_keeps_modest_workload`：`DEFAULT_L2_EMBEDDING_CACHE_CAP=50_000` 下 modest workload 全保留（cache 7/7）；既有 L2 round-trip（TEST-22.2.3）+ L1 cap（TEST-31.2.1）+ 既有 TEST-22.2.* 不退化。**实现校正（ADR-013）**：规划设想把 L2 cap 存入 `with_sqlite` add-only 私有字段；实现采 add-only `with_sqlite_capacity` ctor（既有 `new` / `with_sqlite` / `with_capacity` 公共签名不动，源码兼容）；`embedding_cache` schema + L1 行为不变；0 新 dep（ADR-008）。`cargo test --workspace` 207/0（v0.26.0 平台矩阵）。
- AC3：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威；PR #218 spec-lint PASS，§4 CI 状态表）。
- 0 新 dep / 0 schema migration（隐式 rowid，不加列 / 不 `ALTER`）/ 默认行为不变 / 既有契约不变 / opt-in-path caveat 据实。**HONEST CAVEAT（ADR-013）**：`with_sqlite` 当前无 production call site（test-only）→ 本 L2 bound 是 opt-in 路径的纵深防御（defense-in-depth），**非已确认的 live leak**。true-LRU（access-order 而非 insert-order 淘汰）honest-defer `[SPEC-DEFER:phase-future.l2-cache-true-lru]`——隐式 rowid FIFO 是 insert-order，access-order true-LRU 须额外 last-access 列 + UPDATE 热路径，据真实 trade-off 不强做。实测磁盘上界数值不预填（真实跑出才记数）。

**实际改动文件**（v0.26.0 / PR #218，squash `3ce290a`）：
- `core/src/embedding/cache.rs`——`sqlite_put` 写后加 `COUNT(*)` 同等价的 rowid-FIFO `DELETE`（`WHERE provider=? AND dim=? AND rowid NOT IN (SELECT rowid ... ORDER BY rowid DESC LIMIT cap)`，超 cap 时最旧 rowid 行淘汰）；加 L2 默认 cap 常量 `DEFAULT_L2_EMBEDDING_CACHE_CAP=50_000`（与 `DEFAULT_EMBEDDING_CACHE_CAP` 并列）+ add-only `with_sqlite_capacity` ctor（既有 `with_sqlite` 不动）。+ 同源 test `test_33_1_1_l2_cap_evicts_oldest_fifo`（cap 写溢出 → 最旧 rowid 逐出 round-trip）+ `test_33_1_2_default_l2_cap_keeps_modest_workload`（default cap modest workload 全保留，cache 7/7）。`CREATE TABLE` 不改（用隐式 rowid，0 schema migration）。
