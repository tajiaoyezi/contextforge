...
**§9 Verification 实证**（real evidence）：
- **AC1 qdrant live recall 真实兑现**：task-36.1（#236）harness + task-36.2（#237）`qdrant-recall` CI service-container job 经 **CI run 26961084355** 实测 `recall@10=1.0000`（N=2000 dim=64 M=50，qdrant LIVE KNN == brute-force exact ground truth，service container `qdrant ready after 1 attempt(s)` + `2 passed`）；ADR-041 `Proposed→Accepted`（per-D，D2 真实回填）+ ADR-034 add-only Phase-36 Amendment 标 D2 `[SPEC-DEFER:phase-future.qdrant-server-lifecycle]` fulfilled（不溯改 D-body，ADR-014 D5）。诚实判读（ADR-013）：recall=1.0 = qdrant 低于 HNSW indexing_threshold 服务精确 KNN 的 live 正确性证明，HNSW 近似域大语料 recall 续 `[SPEC-DEFER:phase-future.vector-large-corpus-perf]`。
- **AC2 v0.29.0 closeout**：smoke v26 `[45/45]`（banner v25→v26，staging `cf-v28-cfg`）+ `TestTask363_SmokeV26QdrantLiveRecallStep`（`go test -run TestTask363` PASS，无回归 `[37/37]`..`[44/44]`）+ `bash -n scripts/console_smoke.sh` OK；release docs `docs/releases/v0.29.0-{evidence,artifacts}.md`（真实 recall@10=1.0000 + CI run 26961084355；tag/run/digest/tlog `<backfill>` until post-tag-push）+ README v0.29 段 + RELEASE_NOTES v0.29.0 段；roadmap §3.18 推进记录 + §4 标 qdrant-server-lifecycle 已兑现；adapter Phase 36 Done / Tasks 3 / ADR-041 Accepted。
- **AC3 D2 lint**：`spec_drift_lint --touched origin/master`（CI spec-lint 权威）。

**实际改动文件**：`scripts/console_smoke.sh`（v26 step [45/45] + banner）+ `internal/cli/smoke_syntax_test.go`（TestTask363）+ `docs/releases/v0.29.0-evidence.md` + `v0.29.0-artifacts.md`（新）+ `README.md` + `RELEASE_NOTES.md`（v0.29 段）+ `adr-041`（Accepted + Ratification）+ `adr-034`（add-only Phase-36 Amendment）+ `docs/roadmap.md`（§3.18 + §4）+ `docs/s2v-adapter.md`（Phase 36 + tasks + ADR-041）+ phase-36 spec（Done + §6 [x]）+ 本 task spec。0 生产代码改动（closeout = 文档 + smoke；CI job 已在 task-36.2 #237）。

**真实 release（v0.29.0 tag/run/digest/tlog）**：用户已授权（AskUserQuestion 2026-06-04，ADR-012）；closeout 合入后 push annotated tag → release.yml → post-tag-push backfill 填 `<backfill>`（ADR-013 不预填）。
