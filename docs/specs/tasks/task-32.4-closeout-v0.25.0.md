...
|---|---|---|---|
| TEST-32.4.1 | smoke v22 step `[41/41]`（vector backend config-selectable baseline + factory sqlite-vec arm 标记 + 无回归既有 denominator）+ `bash -n` 过 + `go test -run TestTask324` 过 + v0.25.0 release docs + ADR-037 per-D ratify Accepted（D2 in-process 矩阵 cell / D4 real chunk filter feature honest-defer 如实） | `scripts/console_smoke.sh` + `internal/cli/smoke_syntax_test.go` + release/ADR-037 | Done |
| TEST-32.4.2 | ADR-034 add-only Phase 32 Amendment（sqlite-vec arm 补全工厂后端覆盖，不溯改正文）+ ADR-023 守线引用 + roadmap §3.14 + §4 add-only 新 backlog + phase-32 §6 闭合 + adapter + feature | ADR-034/roadmap/phase/adapter/feature | Done |
| TEST-32.4.3 | D2 lint `--touched origin/master` 0 未标注命中（CI spec-lint 权威）（LAST） | `scripts/spec_drift_lint.sh` | Done |
...
**§9 Verification（docs/s2v-index-consistency-backfill，真实证据）**：
- AC1：`bash -n scripts/console_smoke.sh` exit 0 + `go test ./internal/cli/ -run TestTask324`（smoke 语法 + syntax test，v22 step `[41/41]` + vector-backend-config + factory sqlite-vec arm + TEST-32.1/32.2/32.3 标记 + 无回归 `[37/37]`..`[40/40]`，denominator 不溯改）PASS。v22 step `[41/41]` 为文档/状态步：验默认构建为内部接线/契约修复、无 console-api 运行时面，既有 v13-v21 块 denominator 不溯改（新增 `[41/41]` add-only，ADR-014 D5）。
- AC2：`docs/releases/v0.25.0-{evidence,artifacts}.md` + `README.md` v0.25 段 + `RELEASE_NOTES.md` v0.25.0 段 + ADR-037 Proposed→Accepted（per-D Ratification：D1/D3/D5 Accepted、D2 PARTIAL[arm/双半门控/selection-matrix wiring + 真实 MSVC feat-on 构建 Accepted，in-process recall/latency CELL honest-defer `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]`]、D4 honest-defer 边界重申）+ ADR-034 add-only `## Amendment (Phase 32 / v0.25.0)`（sqlite-vec arm 补全工厂后端覆盖，不溯改 D1-D5 + Phase 29 Ratification 正文）+ ADR-023 守线引用 + `docs/roadmap.md` §3.14 推进记录 + §4 add-only 新 backlog（chunk-source-type-filter / chunk-agent-scope-filter / sqlite-vec-inprocess-matrix）+ `docs/s2v-adapter.md` Phase 32 Draft→Done/Tasks 0→4 + ADR-037 Proposed→Accepted + phase-32 §6 AC 全 `[x]` + Status Done。
- AC3：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- 全 phase 真实验证：默认构建 `cargo test --workspace` = 199 passed, 0 failed + `go test ./...`（含 TestTask324 + TestTask323）全过 + `cargo clippy --workspace --all-targets -- -D warnings` 0 warning + `go vet ./...` clean + gofmt clean + `bash -n scripts/console_smoke.sh` exit 0。feat-on REAL x86_64-pc-windows-msvc `cargo test --features vector-sqlite` 构建通过（arm wiring 真实验证，rustc 1.95.0）。默认构建 0 新依赖 + 0 network + 默认行为/proto 既有字段/既有契约不变（ADR-004/008）。**in-process recall/latency CELL（真实 KNN recall@k + latency on a corpus）honest-defer `[SPEC-DEFER:phase-future.sqlite-vec-inprocess-matrix]` + 真实 chunk source_type/agent_scope filter feature honest-defer `[SPEC-DEFER:phase-future.chunk-source-type-filter]` / `[SPEC-DEFER:phase-future.chunk-agent-scope-filter]`——据 ADR-013 不伪造数值。**

**实际改动文件**（实现 PR #215，squash commit `cb3bb08`）：`scripts/console_smoke.sh`（v22 step `[41/41]`）+ `internal/cli/smoke_syntax_test.go`（`TestTask324`）+ `docs/releases/v0.25.0-{evidence,artifacts}.md`（new）+ `README.md` v0.25 段 + `RELEASE_NOTES.md` v0.25.0 段 + `docs/decisions/adr-037-vector-backend-config-plumbing-and-completeness.md`（ratify Proposed→Accepted per-D）+ `docs/decisions/adr-034-production-vector-live-recall.md`（add-only Phase 32 Amendment）+ ADR-023 守线引用 + `docs/specs/phases/phase-32-vector-backend-config-plumbing-and-completeness.md`（§6 闭合 + Status Done）+ `docs/s2v-adapter.md` + `docs/roadmap.md`（§3.14/§4）+ `test/features/phase-32-vector-backend-config-plumbing-and-completeness.feature` + task-32.4 spec（Status Done + §10）。

**真实 v0.25.0 tag/run/digest/tlog**（post-tag-push backfill PR #216，squash commit `6df983f`；权威来源 `docs/releases/v0.25.0-{evidence,artifacts}.md`）：
- v0.25.0 annotated tag SHA：`cb3bb08ec4f9eafc63805d89cd88c373a0cc2b26`（master @ task-32.4 closeout merge #215）。
- `release.yml` run ID：`26874124924`（conclusion **success**——build-push + cosign keyless sign + cosign attest SPDX SBOM + provenance:mode=max 步全绿）。
...
- cosign Rekor 透明日志 tlog idx：`1708460120`（sign 镜像 digest）/ `1708461746`（attest SPDX SBOM）。
