...
- AC1（grounding 校正 7→3-4）：4 处 survey 候选 DROP/LEAVE 0 生产代码改动如实记录于 §范围外 + ADR-040 Context+D3 + `docs/releases/v0.28.0-evidence.md` §3.3——`search.rs:109` already-surfaced（core 无 metrics facility）/ `mcpadapter/server.go:298` already-done（task-31.3）/ `mcpadapter/allowlist.go:31` 有意 POSIX-only / `index_session_backend.rs:193` eb.send 有意 no-subscribers；不引新 metrics facility。`git diff` 仅 docs/smoke/test，4 处生产代码 0 改动。
- AC2（v0.28.0 closeout）：`bash -n scripts/console_smoke.sh` exit 0 + `go test ./internal/cli/ -run TestTask353` PASS——smoke v25 step `[44/44]`（banner v24→v25，staging `cf-v27-cfg` offset +2，标记 observability-hardening/TEST-35.1.*/TEST-35.2.*/TEST-35.3.*/eprintln/setVectorEnv + 无回归 `[37/37]`..`[43/43]`）。v0.28.0 release docs（evidence/artifacts `<backfill>` + README v0.28 段 + RELEASE_NOTES v0.28.0 段）+ ADR-040 Proposed→Accepted per-D ratify（§Ratification）+ ADR-031 add-only Phase 35 Amendment + ADR-036 add-only 镜像 Amendment + roadmap §3.17 推进记录 + §4 + phase-35 §6 闭合 Status Done + adapter（Phase 35 Done/Tasks 3/ADR-040 Accepted/+35.3/BDD）。
- AC3（D2 lint）：`bash scripts/spec_drift_lint.sh --touched origin/master` 0 未标注命中（CI spec-lint 权威）。
- 全 phase 不退化：`cargo test -p contextforge-core --lib` 212 + `go test ./...` 全过 + `cargo clippy --workspace --all-targets -- -D warnings` 0 warning。

**实际改动文件**：`scripts/console_smoke.sh`（banner v24→v25 + v25 step [44/44] cf-v27-cfg）+ `internal/cli/smoke_syntax_test.go`（`TestTask353`）+ `docs/releases/v0.28.0-{evidence,artifacts}.md`（新，`<backfill>`）+ `README.md` v0.28 段 + `RELEASE_NOTES.md` v0.28.0 段 + `docs/decisions/adr-040-...md`（Proposed→Accepted + Ratification）+ `docs/decisions/adr-031-...md`（add-only Phase 35 Amendment）+ `docs/decisions/adr-036-...md`（add-only 镜像 Amendment）+ `docs/roadmap.md`（§3.17 推进记录 + §4 add-only）+ `docs/specs/phases/phase-35-...md`（Draft→Done + §6 [x]）+ `docs/s2v-adapter.md`（Phase 35 Done/+35.3/ADR-040 Accepted）+ `test/features/phase-35-...feature`（规划已建）。0 生产代码改动（grounding 校正 verify-only + closeout 文档/smoke）。真实 v0.28.0 tag/release 经用户授权（AskUserQuestion 2026-06-04）→ post-tag-push backfill 填实 tag SHA / run id / digest / tlog（`<backfill>`，ADR-013 不预填）。
