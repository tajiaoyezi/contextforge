...
> ✅ 已过 `/s2v-implement` §2A 前置审核（2026-05-17）：§3/§4/§5.2/§5.3 `<TBD-by-user>` 已清零、§6 AC 经用户审定接受。实时状态以下方 `**Status**` 字段为准；状态机见 `docs/s2v/standard.md` §10.5.1。
...
- [x] **AC1** (PRD §Technical Approach Canonical Record v0.1 最小 schema): `ContextRecord` proto 含 PRD 列出的全部最小字段（id / schema_version / collection_id / source_type / source_provider / source_uri / agent_scope / content / content_hash / redaction_status / language / file_path / line_start / line_end / tags / provenance[] / security_labels / created_at / updated_at / expires_at / version / metadata.extra），未识别字段进 `metadata.extra` 不影响核心字段。
- [x] **AC2** (PRD §Technical Approach): 额外定义 `SourceRecord` / `Chunk` / `RetrievalResult` 三类对象的 proto，四类对象边界与字段与 PRD §Technical Approach 一致。
- [x] **AC3** (PRD §Technical Approach REST/MCP 最小接口契约草案): `search` proto 的请求/响应字段与 PRD 草案一致（query/collections/agent_scope/top_k/filters/explain → results[].chunk_id/context_id/source_type/file_path/line_start/line_end/score/retrieval_method/reason/agent_scope/redaction_status/provenance）。
- [x] **AC4** (PRD §Decisions Log D1): proto 可由 Go（grpc-go）与 Rust（tonic）两侧 codegen 成功，无 FFI。
- [x] **AC5** (PRD §Technical Risks R1 / 本 task 新增): proto 与 canonical record 标注 `schema_version="0.1"` 并写明版本化规则「仅加字段、不删不改 tag」（契约冻结声明）。

## 7. SDD / BDD / TDD Traceability

| Acceptance Criterion | BDD Scenario | TDD Test | Integration / E2E Test | Verification | Status |
|---|---|---|---|---|---|
| AC1 ContextRecord 最小字段 | SCEN-1.1.1 | TEST-1.1.1 | - | unit-test | Done |
| AC2 四类对象 proto | SCEN-1.1.2 | TEST-1.1.2 | - | unit-test | Done |
| AC3 search 契约一致 | SCEN-1.1.3 | TEST-1.1.3 | - | unit-test | Done |
| AC4 Go+Rust codegen | SCEN-1.1.4 | TEST-1.1.4 | - | unit-test / typecheck | Done |
| AC5 schema 版本化冻结 | SCEN-1.1.5 | TEST-1.1.5 | - | unit-test | Done |
...
- **完成日期**：2026-05-17
- **改动文件**：
  - `proto/contextforge/v1/context.proto`（新增 — Provenance/SourceRecord/ContextRecord/Chunk + schema_version 冻结声明）
  - `proto/contextforge/v1/search.proto`（新增 — SearchFilters/SearchRequest/RetrievalResult/SearchResponse）
  - `proto/contextforge/v1/service.proto`（新增 — ContextService{Search,Health}；Health 支撑 phase-1 §6 gRPC health）
  - `proto/contextforge/v1/import.proto` · `eval.proto`（新增 — import/eval 最小契约）
  - `proto/contextforge/v1/*.pb.go` + `service_grpc.pb.go`（新增 — buf+protoc-gen-go(-grpc) 生成，committed 供离线验证）
  - `proto/buf.yaml` · `buf.gen.yaml`（新增 — buf 自包含 codegen 管线，无需系统 protoc）
  - `internal/contract/contract.go`（新增 — Go 侧契约 façade：解析 proto SSOT + grpc-go 生成产物 smoke）
  - `internal/contract/contract_test.go`（新增 — TEST-1.1.1~1.1.5）
  - `core/Cargo.toml` · `core/build.rs` · `core/src/lib.rs` · `core/src/contract.rs`（新增/修改 — tonic-build+vendored protoc 构建期 codegen + Rust 侧 façade）
  - `core/tests/proto_contract.rs`（新增 — TEST-1.1.1~1.1.5 Rust 侧）
  - `go.mod` · `go.sum` · `Cargo.toml` · `Cargo.lock`（新增 — 首 task 构建系统 bootstrap，§2A 已透明告知；非 R7 中途加依赖）
- **commit 列表**：
  - `8cae4c2` docs(spec): task-1.1 进入实施 (Status: Ready → In Progress)
  - `5674852` test(proto): 加 SCEN-1.1.1~1.1.5 共 5 个 RED 测试（§2.5.1 可编译骨架）
  - `1c25870` feat(proto): 冻结 gRPC + canonical-record 契约 v0.1，Go+Rust 双侧 codegen 通过全部 10 个测试
  - 本回填 docs(spec) commit 见步 11.A（回填 §10 + Status → Done）
- **§9 Verification 结果**：
...
- **剩余风险 / 未做项**：
  - 步 3 基线为 greenfield 冷启动豁免（首 task，前无源码/测试基建）；§9 install/typecheck/unit-test 均为实施后真实执行且全绿，非 skip。
  - Go 生成产物 `.pb.go` 已 committed（Go 惯例）供离线 `go vet`/`go test`；Rust 生成产物由 `core/build.rs`（tonic-build + vendored protoc）每次构建重生（tonic 惯例，离线可复现，不入库）。codegen 工具 buf/protoc-gen-go(-grpc) 经 `go install` 装于 GOPATH/bin（dev 工具，非项目依赖）。
  - 契约 v0.1 已冻结：schema_version="0.1" + 冻结规则文档化并被 TEST-1.1.5 双侧断言；后续仅可加字段、不删不改 tag（缓解 PRD R1 / 关联 O9）。
  - import/eval proto 为 v0.1 最小占位契约（业务方法实现属后续 phase，本 task §3 Out-of-Scope）。
- **下游 task 影响**：正向地基 — task 1.2/1.3/1.4 及 Phase 2+ 全部消费本 task 冻结的 `proto/contextforge/v1` 契约与 canonical record schema；契约变更须遵守冻结规则（仅加字段、不删不改 tag）。
