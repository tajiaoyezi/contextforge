...
- [ ] AC1（真分词器 feature 🟢 分词 / 🔴 dict dep）: 新 `cjk-segmenter` feature 下真分词 analyzer 把 `配置加载` 切成 `配置`/`加载`（区别 bigram `配置`/`置加`/`加载`）+ 新 analyzer 名双站点注册对称（索引 `open_with_tokenizer:442` + 查询 `open_with_config:250`）opt-in round-trip 命中无静默退化（task-24.1 R4）+ 默认构建（无 feature）default tokenization + 6-field schema + 0 新 dep 不变（🟢 deterministic）；重词典 dep（jieba-rs / lindera）经主 agent R7 chore + ADR-008 add-only（🔴 dict dep，本 phase 规划 NOTE）— verified by TEST-30.1.1 / TEST-30.1.2 / TEST-30.1.3
- [ ] AC2（tokenizer-default-on + migration + 真实 recall delta 🟡）: 既有索引 reindex/migration 工具（默认 analyzer 绑定持久化 meta.json，翻默认须 re-index）+ `RetrieverConfig.tokenizer`:99 路由接线（或文档化 schema-driven 对称）+ 既有默认索引向后兼容不破坏 + 扩展 CJK golden 经 Go `ValidateGoldenSemantic` 校验 + phase24-harness 量 default vs bigram vs 真分词真实 recall delta（🟡 须 feature 构建 + 本地真实跑，真实跑出后回填、小语料不外推，ADR-013）；若 full default-on 迁移过重则诚实延后 default flip + 留 opt-in + 迁移工具 [SPEC-DEFER:phase-future.tokenizer-default-on]— verified by TEST-30.2.1 / TEST-30.2.2 / TEST-30.2.3
- [ ] AC3（默认 0-dep 不变 + v0.23.0 closeout）: 默认构建 default tokenization + 6-field schema + 0 新 dep 维持不变（ADR-004）+ v0.23.0 release docs（evidence/artifacts/README/RELEASE_NOTES，tag/run/digest 待回填）+ `scripts/console_smoke.sh` step `[39/39]`（default build init baseline + default tokenization 不变 + 既有 step 不退化 denominator 不溯改 ADR-014 D5）+ phase §6 闭合 + ADR-035 据真实分词单测 / 真实 recall delta ratify（受阻维度如实记录）+ ADR-029 add-only Amendment（不溯改正文）— verified by TEST-30.3.1 / TEST-30.3.2
- [ ] AC4（ADR-014）: ADR-014 cross-validation gate 全套通过（第二十一次激活）— D1 mapping + D2 lint `--touched origin/master` 0 未标注命中 + D3 verified-by + D4 自治 + D5 历史 Phase 1-29 不溯改 — verified by TEST-30.3.3 + closeout PR body
