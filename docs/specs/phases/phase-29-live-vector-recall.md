...
- [x] **AC1**（vector backend 工厂 + server.rs 热路径注入；🟢 deterministic）：新增 `select_vector_backend(name, dim) -> Result<Arc<dyn VectorStore>, VectorError>`（仿 `embedding/factory.rs::select_provider`；`VectorStore` 为 add-only 组合 trait，IS-A VectorSearcher）——默认 `""`/`"brute"` → `BruteForceVectorBackend`、`"qdrant"`/`"lancedb"` feature off 时返回诚实 Err（不伪造成功）；`server.rs:302`（hybrid）+ `:341`（semantic）经工厂注入替换硬编码 `BruteForceVectorBackend::new()`；默认构建 semantic+hybrid 仍经 BruteForce 工作、`cargo test --workspace` 191 lib + 全集成 0 failed — verified by task-29.1 §6（factory 4/4 PASS）+ phase-smoke step 1（#197）
- [x] **AC2**（qdrant live 端到端 KNN；🔴 live server / 🟢 wiring）：phase29 qdrant harness 经 `QdrantBackend::connect(QdrantConnConfig::from_env())` + `health()` 守门；`cargo build --features vector-qdrant,embedding-fastembed` exit 0，`cargo run`（无 server）→ `health()==Unreachable` honest-defer（实测 eprintln + exit 0）干净通过、**零召回数输出、不伪造**（ADR-013）；Ready 分支真实召回数 真实跑出后回填 §10 + v0.22.0 evidence（NEVER 预填，本 run 无 server → honest-defer）；qdrant 集群 / 副本拓扑 [SPEC-DEFER:phase-future.qdrant-deployment-topology] — verified by task-29.2 §6（wiring + honest-defer PASS）+ phase-smoke step 2（#198）
- [x] **AC3**（lancedb 真实索引 + 多 backend 选择矩阵；🟡 feature build）：feature `vector-lancedb` 下 `create_ann_index` 经 Lance `create_index` 真建 IVF_PQ/IVF_HNSW_SQ 索引并实测召回（IVF_PQ recall@10≈0.44 / IVF_HNSW_SQ≈0.90，真实跑出非预填）；真实多 backend 选择矩阵测量（brute/lancedb 实测；qdrant honest-defer；sqlite-vec 延后）→ ADR-030 D3 + ADR-023 tier add-only Amendment（不溯改正文 D5）；**compaction 真实执行兑现**（1536 行不丢）；broad test rustc ICE → `--lib` scoped [SPEC-DEFER:phase-future.lancedb-build-prereq-ci] — verified by task-29.3 §6（`--lib` 4/4 PASS）+ phase-smoke step 3（#199）
- [x] **AC4**（默认构建 0-dep 基线不变 + v0.22.0 closeout）：默认构建（无 `vector-qdrant`/`vector-lancedb` feature）0 vector 依赖、BruteForce 语义基线不变（ADR-004，`cargo test --workspace` 0 failed）；v0.22.0 release docs（evidence/artifacts/README/RELEASE_NOTES）+ `scripts/console_smoke.sh` v19 step 38（默认构建 init 基线断言 + 既有 step 不退化）+ ADR-034 据真实产物 per-D ratify（D1 工厂+热路径达成 / D2 live-server wiring+honest-defer 部分·真实召回受阻如实 / D3 lancedb 索引+compaction 达成 / D4 矩阵 Amendment / D5 baseline 不变）+ ADR-030/023 add-only Amendment + phase §6 闭合 — verified by task-29.4 §6
- [x] **AC5**：ADR-014 cross-validation gate 全套通过（第二十次激活）— D1 mapping + D2 lint `--touched origin/master` 0 未标注命中 + D3 verified-by + D4 自治 + D5 历史 Phase 1-28 不溯改 — verified by task-29.4 closeout PR body
