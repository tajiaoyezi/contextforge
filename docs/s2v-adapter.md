...
| 29 | `live-vector-recall` | `docs/specs/phases/phase-29-live-vector-recall.md` | Draft | 4 | master（v0.22.0 规划：承 Phase 25 把 qdrant/lancedb 契约层 / 参数层兑现为真实 live 向量召回 — vector backend 工厂 `select_vector_backend` + server.rs:302/341 热路径注入替换硬编码 BruteForce（task-29.1，兑现 `[SPEC-DEFER:phase-future.vector-retrieval-integration]`）+ qdrant live KNN 真实 harness（无 server `health()==Unreachable` 诚实延后，task-29.2，🔴 live / 🟢 wiring）+ lancedb 真实 IVF_PQ/HNSW 索引调参 + 多 backend 选择矩阵真实测量 → ADR-030/023 add-only Amendment（task-29.3，🟡 feature build）+ smoke v19 + closeout（task-29.4）。ADR-034 Proposed；默认构建 0 vector dep + BruteForce 语义 baseline 不变（ADR-004/023 D5）；真实 live KNN / 索引召回 / 矩阵数值一律真实跑出后回填（ADR-013，不预填）；ADR-014 第二十次激活；规划阶段 Draft，实现 + tag/release 另行授权 ADR-012） |
| 30 | `cjk-true-segmenter` | `docs/specs/phases/phase-30-cjk-true-segmenter.md` | Draft | 3 | master（v0.23.0 规划：承 Phase 24 把 CJK 重叠 bigram 升级为真分词器 — 新 `cjk-segmenter` feature（默认 off 0-dep）+ 并行 `cjk_segmenter` analyzer + 双站点注册（index :442 + query :250）对称、bigram 保留 0-dep fallback（task-30.1，🟢 分词单测 / 🔴 重词典 dep）+ tokenizer-default-on 评估 + 既有索引 reindex/migration + `RetrieverConfig.tokenizer`（现 vestigial 恒不读）路由接线或文档化 schema-driven + 扩展 CJK golden 真实 recall delta（task-30.2，🟡 不预填 ADR-013）+ smoke v20 + closeout（task-30.3）。ADR-035 Proposed；真分词器 optional dep（jieba-rs/lindera）经主 agent R7 chore + ADR-008 add-only；默认构建 0 新 dep + 默认 tokenization 不变（ADR-004）；ADR-014 第二十一次激活；规划阶段 Draft，实现 + tag/release 另行授权 ADR-012） |
| 31 | `governance-debt-cleanup` | `docs/specs/phases/phase-31-governance-debt-cleanup.md` | Draft | 4 | master（v0.24.0 规划：清跨 Phase 治理债 — Go fallback `MemMemoryStore` memory 变更 emit `memory.*` event 对齐 workspace/job + Rust 路径 + event-bus partition/capacity **经核 Phase 26 已交付** verify-only + roadmap §4 add-only 更正（task-31.1，🟢，不重复实现 ADR-013）+ embedding-cache LRU + Go memstore cap 可配置 + compose `mem_limit`/`cpus`/可选 TLS proxy（task-31.2，🟢 / 🟡 真实 cert）+ eval case-results 子表（add-only migration 0018）+ exporter `content=""` 经新 `ListAllChunks` RPC 真实全文 + 3 MCP nits + C2/C3/C4 诚实延后重申（task-31.3，🟢）+ smoke v21 + closeout（task-31.4）。ADR-036 Proposed；ADR-021/027/029/033 add-only Amendment（不溯改正文 D5）；默认行为 + 既有契约（proto/migration add-only、cache 默认值、compose 可选）不变（ADR-004）；ADR-014 第二十二次激活；规划阶段 Draft，实现 + tag/release 另行授权 ADR-012） |
...
| 034 | production-vector-live-recall | Proposed | docs/decisions/adr-034-production-vector-live-recall.md |
| 035 | cjk-true-segmenter-and-tokenizer-default | Proposed | docs/decisions/adr-035-cjk-true-segmenter-and-tokenizer-default.md |
| 036 | governance-debt-cleanup | Proposed | docs/decisions/adr-036-governance-debt-cleanup.md |
