...
| 28 | `release-ci-hardening` | `docs/specs/phases/phase-28-release-ci-hardening.md` | In Progress | 1 | master（v0.21.0 规划：发布 / CI 硬化 — multi-arch 镜像（QEMU + `platforms: amd64,arm64` manifest list）+ 匿名可拉取验证（task-28.1）+ cosign keyless 签名 + SBOM + provenance attestation（task-28.2）+ CI 强 lint（clippy/gofmt 先测存量再定卡红时机，task-28.3）+ smoke v18 + 收口（task-28.4）。ADR-033 Proposed（落地后据真实 CI/release run 产物 ratify，arm64 emulation/Fulcio·Rekor/lint 存量受阻则诚实延后不强 ratify，ADR-013）；ADR-007 部署发布面扩展 add-only Amendment（不溯改正文 D5）；纯 `.github/workflows/*` + lint 修复层，镜像运行时 + 默认 0-network/0-dep baseline 不变（ADR-004）；ADR-014 第十九次激活。spec Draft，task spec 待实施时 `/s2v-add task` 创建；tag/release outward-facing 须用户显式授权（ADR-012）） |
...
| 28.1 | `.github/workflows/release.yml` setup-qemu-action + `platforms: amd64,arm64` manifest list + `verify-image.yml` 未鉴权匿名 pull 守护 + 多架构 manifest assert + index digest parity | docs/specs/tasks/task-28.1-multi-arch-image-and-anonymous-pull.md | Draft | Phase28 #1 | master |
