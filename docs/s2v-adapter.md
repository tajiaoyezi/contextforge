...
| 33 | `governance-debt-cleanup-2` | `docs/specs/phases/phase-33-governance-debt-cleanup-2.md` | Draft | 4 | plan/phase-33-governance-debt-cleanup-2（v0.26.0 规划：第二轮治理债清扫，镜像 Phase 31 — L2 SQLite embedding-cache rowid-FIFO 有界（task-33.1，0-dep/0-migration，opt-in-path 防御）+ memstore FIFO→access-order LRU + hard-delete no-dangling-ref 不变式（task-33.2，cascade 经核非问题 honest-defer / handleMemoryPin lenient ADR-022 D2 据实不改）+ observability indexing.* 持久化(migration 0019)+replay mapper + TraceStore 多 workspace 隔离 add-only proto + drain verify-only（task-33.3，🟢/🟡）+ export --timeout + closeout（task-33.4，%v→%w non-bug / tracestore-fts 已修复 / datadir env→Options honest-defer）。ADR-038 Proposed；默认行为+契约+0-dep 不变（ADR-004/008）；grounding 据实下修多处 survey 过陈（ADR-013）；实现 + tag/release 经用户授权 ADR-012） |
...
| 038 | governance-debt-cleanup-2 | Proposed | docs/decisions/adr-038-governance-debt-cleanup-2.md |
