...
| 19.7 | Phase 19 closeout（端到端语义检索 ship，路径 A）+ v0.12.0 release docs（README/RELEASE_NOTES/evidence/artifacts）+ phase-19 §6 AC1-6 [x] + Status Done + adapter Phase 0→7/forward-ref 解除 + tag（用户授权待 push）+ backfill | docs/specs/tasks/task-19.7-closeout-v0.12.0.md | Done | Phase19 #7（dep 19.1-19.6；v0.12.0 = 端到端语义检索 live + ADR-023 Accepted；tag push USER-AUTHORIZED-ONLY）| master |
