...
| 28 | `release-ci-hardening` | `docs/specs/phases/phase-28-release-ci-hardening.md` | In Progress | 2 | master（v0.21.0 规划：发布 / CI 硬化 — 匿名可拉取验证 + multi-arch（arm64 emulation 实测不可行→延后原生 runner，task-28.1 Done）+ 供应链证明（GitHub 原生 attestation：attest-build-provenance + attest-sbom，task-28.2；ADR-033 §D2 由 cosign 细化）+ CI 强 lint（clippy/gofmt 先测存量再定卡红时机，task-28.3）+ smoke v18 + 收口（task-28.4）。ADR-033 Proposed（落地后据真实 CI/release run 产物 ratify，arm64 emulation/Fulcio·Rekor/lint 存量受阻则诚实延后不强 ratify，ADR-013）；ADR-007 部署发布面扩展 add-only Amendment（不溯改正文 D5）；纯 `.github/workflows/*` + lint 修复层，镜像运行时 + 默认 0-network/0-dep baseline 不变（ADR-004）；ADR-014 第十九次激活。spec Draft，task spec 待实施时 `/s2v-add task` 创建；tag/release outward-facing 须用户显式授权（ADR-012）） |
...
| 28.2 | `release.yml` GitHub 原生签名 attestation（attest-build-provenance SLSA + attest-sbom syft SPDX，push 证明库+GHCR referrer）+ `verify-image.yml` gh attestation verify（ADR-033 §D2 由 cosign 细化为原生 attestation） | docs/specs/tasks/task-28.2-image-signing-sbom-provenance.md | Draft | Phase28 #2（dep 无，可与 28.3 并行；真证明在 v0.21.0 release） | master |
...
| 033 | release-ci-hardening | Proposed | docs/decisions/adr-033-release-ci-hardening.md |
