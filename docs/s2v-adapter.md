...
- **Lint**: cargo clippy --workspace --all-targets -- -D warnings && go vet ./... && test -z "$(gofmt -l .)"
- **Typecheck**: go vet ./... && cargo check --workspace
- **Unit Test**: go test ./... && cargo test --workspace
- **Integration tests**: N/A: Rust 集成测试（core/tests/*.rs）随 Unit Test 的 cargo test --workspace 执行；env-gated live harness（qdrant_live_recall / remote_embedding_recall / remote_rerank_recall）见 CI qdrant-recall job 与 §Constraints
- **E2E tests**: bash scripts/console_smoke.sh
- **Build**: cargo build --workspace && go build ./...
- **Coverage**: N/A: 无 CI coverage gate（§Coverage 判读规则 记录手动 Go go test -cover ./... / Rust cargo tarpaulin 判读法）
- **Runtime smoke**: bash scripts/release_smoke.sh
