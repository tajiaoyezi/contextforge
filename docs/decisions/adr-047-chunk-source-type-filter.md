...
- **D4（agent_scope honest-defer memory 层概念续 no-op）→ Accepted 🟢**：`agent_scope` 经 grounding 为 memory 层概念（`memory_items` 0013 / `MemoryListFilter` / `ListMemory` scope / `memstore.go:629-635`），chunks 无 agent 关联、无可派生维度 → retriever 窄化 no-op 块仅覆盖 agent_scope（非空 → byte-equiv，TEST-42.1.2 守护），`[SPEC-DEFER:phase-future.chunk-agent-scope-filter]` 据实保持（不伪造，ADR-013）。smoke v32[51/51]（REAL：`index-job-real` fixture 全 markdown，`runner`/JobRunner 文档于 .md → source_type=doc → `?source_type=doc` **保留** JobRunner doc hit / `?source_type=code` **过滤掉它**，distinguishing；no-op 会两者皆返回）+ TestTask423（无 [37/37]..[50/50] 回归）。默认构建 0 新 dep（`classify_source_type` 纯 std）+ 0 网络 + 0 schema migration + 空 filter byte-equiv；`cargo test --workspace`（core lib 225）+ `go test ./...` 三门不退化。
