...
**Status**: Accepted（v0.24.0 / task-31.4 ratify；D2 真实 TLS cert / D4 native-runner·attestation 受阻维度 honest-defer 部分 ratify）
...
## Ratification (v0.24.0 / task-31.4)

本 ADR 于 v0.24.0 closeout（task-31.4）据 task-31.1/31.2/31.3 的**真实 CI / 实测产物** ratify **Proposed → Accepted**（ADR-013：禁据合成/伪造 ratify）。逐 D 真实依据：

- **D1（memstore-event Go parity + event-bus verify-only 更正）→ ✅ Accepted**：task-31.1（PR #206）`MemStore.EmitEvent` + `MemMemoryStore.SetEventSink`，Pin/Deprecate/SoftDelete/Unpin/HardDelete 发 `memory.*`（event_type 对齐 Rust `audit_op_to_event_type`，Pin/Unpin 共 memory.pin）；`console_api_serve.go` 接线。`go test -run TestMemMemoryStore_EventParity` PASS。event-bus partition/capacity verify-only：`cargo test data_plane::events` **6 passed**（含 TEST-26.3.1a/b/c，Phase 26 已交付）；roadmap §4 add-only 更正（规划 PR #196 line 294 已落地）。
- **D2（cache + deploy hardening）→ 🟡 PARTIAL（code-local Accepted；真实 TLS cert honest-defer）**：task-31.2（PR #207）`BoundedCache`（FIFO-on-insert，0 新 dep，DEFAULT_EMBEDDING_CACHE_CAP=50_000）`cargo test embedding::cache` 5 passed；`memstore.go` `resolveCacheCapacity`（env `CONTEXTFORGE_CONSOLEAPI_CACHE_CAP`）`go test -run TestMemStoreCacheCap` PASS；compose `mem_limit`/`cpus` + `profiles:[tls]` caddy 反代——`docker compose config` + `--profile tls config` **实测 parse OK**。**真实 TLS cert 签发须真实域名 → honest-defer `[SPEC-DEFER:phase-future.compose-tls-auto-cert]`，不伪造**（ADR-013）。
- **D3（eval 子表 + exporter 全文 + MCP nits）→ ✅ Accepted**：task-31.3（PR #208）migration 0018 `eval_case_results` 子表 + 双写 + `query_case_results`/`case_pass_ratio` SQL 查询（`cargo test eval::store` 12 passed）；`ListAllChunks` add-only RPC（proto + buf + Rust handler 经 enumerate_chunks 读真实全文 + daemon client + main.go 接线 + exporter ChunkLoader）→ exporter content 非空 + 真实 ContentHash（`go test exporter` PASS）；3 MCP nits（protocolVersion 日期解析 forward-compat / audit.Write err warn / allowlist mode warn，`go test mcpadapter` PASS）。add-only proto 不破 Search/SearchResponse。
- **D4（诚实延后维度重申）→ ✅ Accepted（honest-defer，非 code）**：rust-native-eval-runner（无 consumer）/ multi-arch-native-runner（QEMU emulation 不可行，须原生 arm64 runner）/ github-native-attestation（私有仓库 `actions/attest-*` 不可用，cosign 在用）三项 task-31.3 §3 范围外 `[SPEC-DEFER]` 重申延后，本 task ADR-033 add-only Amendment 记录，**不伪造完成**（ADR-013）。
- **D5（默认行为 + 既有契约不变）→ ✅ Accepted**：`cargo test --workspace` 0 failed + `cargo clippy --workspace --all-targets -- -D warnings` 0 warning + `go test ./...` exit 0 + `go vet` 0；exporter ListAllChunks add-only RPC / cache cap 默认值不变 / compose 限值可选 / eval 子表 add-only migration / MCP nits 不破协议；默认行为 / proto 既有字段 / 既有契约不变。

ratify 范围 = memstore parity + event-bus verify-only（D1）+ cache/deploy code-local 硬化（D2 partial，真实 cert honest-defer）+ eval 子表/exporter 全文/MCP nits（D3）+ 诚实延后重申（D4）+ 默认 baseline 不变（D5）。**真实 TLS cert / native arm64 runner / github-native-attestation 据「已达维度 ratify + 受阻维度如实记录」honest-defer，不伪造**（ADR-013）。证据见 `docs/releases/v0.24.0-evidence.md` §3。

## Amendment (Phase 35 / v0.28.0, 2026-06-04 — add-only, 正文不溯改)

task-31.3（D3 nit 2/3）确立的 Go **`fmt.Fprintf(os.Stderr)` best-effort audit-surfacing 模式**（`writeAudit` / `allowlist.go` 权限告警，把被吞的错误 surface 到 stderr 而不阻断 RPC happy-path）即 Phase 35（ADR-040 observability-hardening）task-35.2 镜像之源——`setVectorEnv` 的 `config.Load`/`os.Setenv` 失败据同一模式经 `fmt.Fprintf(os.Stderr)` 显式化（`errors.Is(os.ErrNotExist)` 守护 missing 静默）。此外 Phase 35 D3 grounding 校正确认本 ADR 修过的 `internal/mcpadapter/server.go:298`（task-31.3 已 surface）+ `allowlist.go:31`（有意 POSIX-only 平台 caveat）属 already-done / intentional → DROP 不返工。以 add-only 记，**不溯改 D1-D5 正文**（ADR-014 D5）；0 新 dep。详见 ADR-040 Ratification + `docs/releases/v0.28.0-evidence.md`。
