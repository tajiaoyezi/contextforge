...
## Amendment (Phase 44 / v0.37.0) — unpin actor 透传 + audit/event source 归因维度兑现 (add-only)

> add-only Amendment（不溯改本 ADR D-body / Ratification (v0.20.0) / Amendment (Phase 40)，ADR-014 D5）。承本 ADR §D1（pin actor/timestamp first-class store 字段）+ Phase 40 Amendment（memory-actor-propagation 入口透传——pin 端）的 unpin 对称维度。

Phase 44 / v0.37.0（ADR-049 D1/D2/D3）兑现 unpin actor 透传 + audit/event source 归因维度——本 ADR §D1 + Phase 40 task-40.1 给 pin 加了 actor 透传（store `pinned_by`），但 unpin 漏了（unpin handler 硬编码 "console-api"）。grounding 发现 store `set_pinned_with_actor(pinned=false)` 丢弃 actor（清 `pinned_by`）→ 单纯透传到 store 是"空透传"；真实落点是 `emit_audit_and_event`（不携 actor + source 硬编码）。task-44.1（PR #280，master @ `8f6e94f`）闭环：unpin handler 透传 actor + `emit_audit_and_event` 加 actor 参数（audit/event source 归因）+ pin 顺带闭环（add-only byte-equiv）+ Go 透传链 + proto `UnpinMemoryRequest` add-only `actor=2`。

本 ADR §D1 的 pin first-class store 字段仍是正确且必要的前序；Phase 44 补「unpin 对称透传 + audit/event source 归因」增量。**诚实校正（ADR-013）**：本轮交付**调用方透传**（audit/event source 归因）；认证身份续 `[SPEC-DEFER:phase-future.memory-actor-authenticated-identity]`；deprecate/softdelete/harddelete actor 透传续 `[SPEC-DEFER:phase-future.memory-actor-all-rpc]`（本 phase 仅做 emit_audit_and_event actor 参数共用基础）。验证 TEST-44.1.1/.2/.3/.4。详见 ADR-049 Ratification (v0.37.0) + `docs/releases/v0.37.0-evidence.md`。
