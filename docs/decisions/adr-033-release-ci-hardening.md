...
**Status**: Accepted (2026-06-02。v0.21.0 closeout（task-28.4）据 task-28.1/28.2/28.3 真实 CI / release run 产物 ratify Proposed→Accepted（ADR-013）；逐维如实：D1 anon-pull 守护真实达成 + arm64 multi-arch emulation 实测不可行 **DEFERRED**、D2 cosign 机制本地 registry 已验证·真实 GHCR 签名于已授权 v0.21.0 release run、D3 lint 门真实达成绿、D4 baseline 不变达成。见 §Ratification。)
...
- **Follow-ups**: multi-arch 原生 runner / 交叉编译替代 QEMU emulation `[SPEC-DEFER:phase-future.multi-arch-native-runner]`；签名密钥轮换 / 私有 KMS / Rekor 自托管 `[SPEC-DEFER:phase-future.signing-key-management]`；clippy / gofmt 存量告警全量清零（若 D3 选 warn-first）`[SPEC-DEFER:phase-future.lint-backlog-cleanup]`；GitHub 原生 attestation（仓库改公开 / 升 GHEC 后）`[SPEC-DEFER:phase-future.github-native-attestation]`；镜像瘦身 / distroless 运行时基座 `[SPEC-DEFER:phase-future.image-slim-distroless]`；SLSA L3+ / 可复现构建 `[SPEC-DEFER:phase-future.reproducible-build-slsa]`。ADR-007 部署发布面扩展以 add-only Amendment 记录（task-28.4，不溯改 ADR-007 正文，ADR-014 D5）。

## Ratification（v0.21.0 / task-28.4，2026-06-02）

v0.21.0 closeout（task-28.4）据 task-28.1/28.2/28.3 的**真实 CI / release run 产物** ratify `Proposed → Accepted`（ADR-013：禁据合成 / 伪造 ratify）。按 §Consequences Ratification 条款「已达维度 ratify + 受阻维度如实记录」逐 D 项：

- **D1（multi-arch 镜像 + 匿名可拉取验证）— 部分 Accepted（anon-pull 达成；arm64 DEFERRED）**：**匿名可拉取守护真实达成**——`verify-image.yml` add-only 未鉴权（`docker logout ghcr.io` 后）pull 步，run `26788773926` **success**（对现有公开 `:latest` `docker pull` exit 0，守 v0.10.0 PRIVATE→403 回归）。**multi-arch（arm64）经实测不可行 DEFERRED**——run `26757640892` `platforms: linux/amd64,linux/arm64` `push:false` 构建 45min42s 超时取消（arm64 QEMU emulation 仍在编译 Rust 依赖树，完整估 75-90min），按 §Negative/open + R1 stop-condition `release.yml` 保持单架构 amd64（不伪造「multi-arch 成功」），arm64 延后原生 runner / 交叉编译 `[SPEC-DEFER:phase-future.multi-arch-native-runner]`。
- **D2（镜像供应链证明：cosign 签名 + SBOM + provenance）— Accepted（机制已验证；真实 GHCR 签名于 v0.21.0 release run）**：初采 GitHub 原生 attestation 实测在用户私有仓库不可用（run `26789731232` failure：`Feature not available for user-owned private repositories`）→ 改 **cosign-为中心**（公共 Sigstore + GHCR OCI 工件，与 repo 可见性无关，即本 ADR §D2 原文意图）。cosign keyless sign + `cosign attest --type spdxjson`（syft SBOM）+ build-push `provenance:max` + `verify-image.yml` cosign verify + verify-attestation 全机制经本地 `registry:2` run `26799480280` **success** 端到端验证。真实 GHCR 镜像签名 / attestation 于**已授权的 v0.21.0 release run**产生（post-tag-push backfill 填实 digest，见 `docs/releases/v0.21.0-evidence.md §7`）。
- **D3（CI 强 lint：先测存量再定卡红）— Accepted**：先实测存量（CI/LF 权威——本机 Windows `autocrlf` 致 `gofmt -l` 96 假阳性，初判误断 gofmt=0 经 PR #190 首次 CI 纠正为 **15 真实** gofmt 文件）：gofmt 15 / go vet 0 / clippy ~33，均小量。全修到全绿（gofmt `-w`+strip 管道；clippy `--fix`+手动+2 targeted allow：生成代码 `clippy::all` + `EventBus::send` `result_large_err`）→ `cargo clippy --workspace --all-targets -- -D warnings` exit 0 + `cargo test --workspace` 全过（core lib 187 passed）；`ci.yml` add-only `lint` job（clippy + gofmt + go vet 三阻断）于 PR #190 run `26820737785` 四门绿。存量小故修完即卡红（非 warn-first）。
- **D4（镜像运行时 / 默认构建 baseline 不变 + outward-facing 授权红线）— Accepted**：全部改动限于 `.github/workflows/*` + surgical clippy/gofmt 修复；镜像运行时行为 + 默认构建 0-network / 0 新依赖 baseline（ADR-004）+ Console Contract v1 不变；既有 cargo-test/go-test/spec-lint 三门不退化；0 新代码依赖（cosign/syft 为 CI 工具）。tag / release outward-facing 不可逆 → 主 agent 不自行 tag，须用户显式授权（ADR-012）。

证据见 `docs/releases/v0.21.0-evidence.md`。

### ADR-007 add-only Amendment（部署发布面扩展，不溯改正文，ADR-014 D5）

ADR-007（minimal-tarball-distribution）的部署发布基线经本 phase 扩展，以 ADR-007 add-only Amendment 记录扩展结果（不溯改 ADR-007 §Decision/§Consequences 正文）：

- 「最小 x86_64 tarball + 单架构镜像」→ 扩展到 cosign keyless 签名 + SPDX SBOM + SLSA provenance 的 OCI 镜像（task-28.2）+ 匿名可拉取守护（task-28.1）；multi-arch（arm64）延后 `[SPEC-DEFER:phase-future.multi-arch-native-runner]`。
- v0.1 tarball 分发基线本身不变（add-only 演进，ADR-007 §Decision L39「加法式演进，演进时新开 ADR」预见）。

详见 `docs/decisions/adr-007-minimal-tarball-distribution.md` §Amendment (Phase 28 / v0.21.0)。

## Amendment (Phase 31 / v0.24.0, 2026-06-03 — add-only, 正文不溯改)

Phase 31（ADR-036 D4）以 add-only 方式**诚实重申**本 ADR 两项受阻延后维度 + 邻接一项，**不溯改正文**（ADR-014 D5）；据 ADR-013 不伪造完成：

- **multi-arch-native-runner（续延后）**：`release.yml` 仍单 `linux/amd64`，arm64 QEMU emulation 实测不可行（ADR-033 D1 DEFERRED 实测确认），须原生 arm64 runner → `[SPEC-DEFER:phase-future.multi-arch-native-runner]` 重申。
- **github-native-attestation（续延后）**：`actions/attest-*` 在 user-owned 私有仓库不可用（task-28.2 run 26789731232 failure 实测），cosign keyless 在用、verify-image 已 PR #193 修（按 digest 验签）→ `[SPEC-DEFER:phase-future.github-native-attestation]` 重申（仓库转 public / 升 GHEC 后可评估）。
- **rust-native-eval-runner（邻接，续延后）**：`core/src/eval/runner.rs` 占位无 consumer，Go harness 为召回口径单一事实源 → `[SPEC-DEFER:phase-future.rust-native-eval-runner]` 重申。

task-31.3 §3 范围外各带 `[SPEC-DEFER]` tag 重申；非 code AC，不伪造完成。详见 ADR-036 §D4 Ratification + `docs/releases/v0.24.0-evidence.md`。
