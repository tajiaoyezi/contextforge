...
## Amendment (Phase 41 / v0.34.0, 2026-06-07 — tokenizer-default-on，add-only，正文 / Phase 30·31 Amendment 不溯改)

Phase 30 Amendment 把 `[SPEC-DEFER:phase-future.tokenizer-default-on]` 标为「🟡 部分兑现（迁移工具 + 评估；full default flip 仍延后——翻默认是产品决策）」。**Phase 41（ADR-046，Accepted）做出该产品决策、兑现剩余的 default flip 维度**，**不溯改本 ADR 正文 / D1-D5 / Phase 30·31 Amendment**（ADR-014 D5）：

- **`[SPEC-DEFER:phase-future.tokenizer-default-on]` → 🟢 兑现（default flip）**：`code_cjk`（D1 的纯 std 0-dep code/CJK analyzer）从 opt-in 翻为**新建 collection 的生产默认**——task-41.1 `core/src/server.rs` `resolve_tokenizer()`（unset → `code_cjk` 翻默认 / `CONTEXTFORGE_TOKENIZER=default` opt-out 回 legacy `TEXT` / unknown·feature-off → stderr WARN + `code_cjk`）+ 生产索引两调用点（`server.rs:141` `CoreService::index` / `jobs/index_session_backend.rs:151`）`open_with_tokenizer(.., &resolve_tokenizer())`；`IndexSession::open`（库便捷入口）/ `DEFAULT_TOKENIZER` 常量不动（既有库调用方 + D1 单测不破）。既有 collection 经 `open_in_dir` 读回持久化 schema **自动安全**（不被静默失效，TEST-41.1.2 守护）；既有 collection 升级由用户经 Phase 30 Amendment 的 `reindex_with_tokenizer` 主动触发（不自动迁移）。task-41.2 加 Go `[retrieval] tokenizer` config + `setTokenizerEnv` env 桥（opt-out / override 通道，env-wins，无段 → core 默认 `code_cjk`）。
- **首次刻意默认变更（据实，非 byte-equiv）**：新建 collection `content` 倒排词项 `TEXT`→`code_cjk`——是项目首次刻意默认行为变更，由 **ADR-046 显式承接**（既有 collection 安全 + opt-out byte-equiv + 不自动迁移 + 实测 justify）。**实测 recall delta +0.1250 recall@5/@10**（default 0.8750 → code_cjk 1.0000，over 当前 16-题 golden——与 Phase 30 Amendment D4 测量 delta(seg−default)=+0.1250 一致；本 ADR D1/§Ratification 的 +0.0909 系 Phase 24 原始 11-题 golden）。jieba `cjk_segmenter` 默认仍不取（0-dep baseline + Phase 30 实测 vs bigram delta=+0.0000）。
