...
## Amendment (Phase 39 / v0.32.0) — console-api-hybrid-forward fulfilled (add-only, 不溯改 D-body / Ratification)

Phase 39（console-api-retrieval-signal-forward，ADR-044）经 console_data_plane proto add-only（`SearchRequest.hybrid=8` + `SearchResultItem.hybrid_score=17`，既有字段号 1-7 / 1-16 冻结，ADR-015 D1）+ `core/src/data_plane/search.rs` `query()` hybrid dispatch 分支（复用本 ADR 的 `search_hybrid` RRF 融合 + `retrieval_method="hybrid"` + `hybrid_score`，**0 backend 算法改动**）+ Go console-api 转发（`contractv1.SearchRequest.Hybrid` + `handleSearch` `?hybrid` OR-merge + `grpcclient` 转发/映射），把本 ADR §Follow-ups 记录的 `[SPEC-DEFER:phase-future.console-api-hybrid-forward]`（对外 console-api `?hybrid=true` REST 转发，承 Phase 20 `?semantic` 范式）**兑现**——对外 `POST /v1/search`（body `{"hybrid":true}` 或 `?hybrid=true`）首次贯通到 core hybrid 路径，响应携 `retrieval_method="hybrid"` + `hybrid_score` 融合分 provenance（与 `v1 RetrievalResult.hybrid_score=15` parity）。task-39.1（#252）+ task-39.2（#253）合入 master，TEST-39.1.1 / 39.1.2 + TEST-39.2.1 / 39.2.2 本地绿、CI 各 14/14；默认 `hybrid=false` 字节等价（ADR-004）。

本 Amendment 仅标记该 follow-up fulfilled，**不溯改 ADR-025 D-body 正文 / `## Ratification Amendment (task-21.3)` / 本 ADR 的 hybrid 融合质量 ratify**（ADR-014 D5）。hybrid 融合质量本 ADR 已 ratify（top-1 0.6667 / MRR 0.7881），Phase 39 是其**对外可达性贯通**、非重测融合质量。
