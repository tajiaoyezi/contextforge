...
**Status**: Proposed（v0.28.0 / task-35.3 closeout 据真实 CI 逐 D ratify；本阶段为 DRAFT/PLANNING，ratification 在 task-35.3 closeout 完成、Proposed 阶段不 ratify、不预填证据——见 §Ratification）
...
- **Ratification**: 本 ADR **Proposed**。task-35.1 / 35.2 通过后于 v0.28.0 closeout（task-35.3）据真实 CI / 实测产物逐 D ratify Proposed→Accepted（ADR-013：禁据合成 / 伪造 ratify；不为 Rust 伪造 stderr-assert 证据）；memstore nil-sink 🟡 impl-grounding 维度据实施时 grounding 结论据实记录（KEEP-with-assertion 或 DROP-as-non-issue），不强 ratify。
...
> 本节为 DRAFT/PLANNING placeholder——本 ADR 现为 **Proposed**，ratification 在 v0.28.0 closeout（task-35.3）据 task-35.1 / 35.2 / 35.3 真实 CI（四门绿：cargo-test / go-test / lint / spec-lint）逐 D ratify Proposed→Accepted。各 D 真实依据于 closeout 据真实产物回填（ADR-013 不预填、不据合成 ratify、不为 Rust 伪造 stderr-assert）。真实 v0.28.0 tag/run/digest 经用户授权后由 post-tag-push backfill 填实（release docs `<backfill>`）。

待实施回填（task-35.3 closeout 据真实 CI / 实测产物逐 D ratify）。
