...
**Status**: Proposed (2026-05-31；待 v0.19.0 closeout（task-26.3）据 task-26.1/26.2 真实非合成验证 ratify Proposed→Accepted，ADR-013：禁据合成 / 伪造 ratify。某维度受阻则据「已达维度 ratify + 受阻维度如实记录」处理，不强 ratify。见 §Ratification（待回填）。)
...
## Ratification（v0.19.0 / task-26.3，待回填）

> 本段在 v0.19.0 closeout（task-26.3）据 task-26.1/26.2 的**真实非合成验证**回填 Proposed→Accepted 的具体依据（ADR-013：禁据合成 / 伪造 ratify）。D1–D6 各项的真实验证依据（FTS 检索往返 / VACUUM 回收 / SSE 帧契约 / 重放顺序契约 / event-bus 配置 / 默认 0-dep）实施完成后逐条回填；某维度受阻则据「已达维度 ratify + 受阻维度如实记录」处理。证据见 `docs/releases/v0.19.0-evidence.md`（待产出）。
